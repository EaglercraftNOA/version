...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
   ...
   public NetHandlerPlayServer(MinecraftServer server, NetworkManager networkManagerIn, EntityPlayerMP playerIn, GamePluginMessageProtocol eaglerProtocol) {
      this.server = server;
      this.netManager = networkManagerIn;
      networkManagerIn.setNetHandler(this);
      this.player = playerIn;
      playerIn.connection = this;
      ServerMessageHandler servermessagehandler = ServerMessageHandler.createServerHandler(eaglerProtocol.ver, this);
      if (eaglerProtocol.ver >= 5 && networkManagerIn instanceof IntegratedServerPlayerNetworkManager) {
         IntegratedServerPlayerNetworkManager integratedserverplayernetworkmanager = (IntegratedServerPlayerNetworkManager)networkManagerIn;
         this.eaglerMessageController = new InjectedMessageController(eaglerProtocol, servermessagehandler, GamePluginMessageConstants.SERVER_TO_CLIENT, integratedserverplayernetworkmanager::injectRawFrame);
         integratedserverplayernetworkmanager.setInjectedMessageController((InjectedMessageController)this.eaglerMessageController);
      } else {
         this.eaglerMessageController = new LegacyMessageController(eaglerProtocol, servermessagehandler, GamePluginMessageConstants.SERVER_TO_CLIENT, (ch, msg) -> this.sendPacket(new SPacketCustomPayload(NetHandlerPlayClient.encodeEaglerLegacyChannel(ch), msg)));
      }
   }
   ...
   public void sendPacket(Packet<?> packetIn) {
      this.sendPacket(packetIn, (PacketSendListener)null);
   }

   public void sendPacket(Packet<?> packetIn, @Nullable PacketSendListener futureListeners) {
   ...
   public void processUpdateSign(CPacketUpdateSign packetIn) {
      ...
      if (worldserver.isBlockLoaded(blockpos)) {
         ...
         for(int i = 0; i < astring.length; ++i) {
            tileentitysign.setText(i, new TextComponentString(TextFormatting.getTextWithoutFormattingCodes(astring[i])));
         }
   ...
   public void processCustomPayload(CPacketCustomPayload packetIn) {
      if (this.eaglerMessageController instanceof LegacyMessageController) {
         String s = NetHandlerPlayClient.decodeEaglerLegacyChannel(packetIn.getChannelName());
         if (s != null) {
            try {
               ((LegacyMessageController)this.eaglerMessageController).handlePacket(s, packetIn.getBufferData());
