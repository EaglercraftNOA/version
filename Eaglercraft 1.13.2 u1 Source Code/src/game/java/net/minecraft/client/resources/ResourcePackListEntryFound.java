...
public class ResourcePackListEntryFound extends GuiListExtended.IGuiListEntry<ResourcePackListEntryFound> {
   ...
   public void drawEntry(int entryWidth, int entryHeight, int mouseX, int mouseY, boolean p_194999_5_, float partialTicks) {
      ...
      if (this.func_195024_j() && (this.client.gameSettings.touchscreen || p_194999_5_)) {
      ...
      int j1 = this.client.fontRenderer.getStringWidth(s);
      if (j1 > 157) {
         s = this.client.fontRenderer.trimStringToWidth(s, 157 - this.client.fontRenderer.getStringWidth("...")) + "...";
      }

      this.client.fontRenderer.drawStringWithShadow(s, (float)(j + 32 + 2), (float)(i + 1), 16777215);
      List<String> list = this.client.fontRenderer.listFormattedStringToWidth(s1, 157);

      for(int i1 = 0; i1 < 2 && i1 < list.size(); ++i1) {
         this.client.fontRenderer.drawStringWithShadow(list.get(i1), (float)(j + 32 + 2), (float)(i + 12 + 10 * i1), 8421504);
      }

   }

   protected boolean func_195024_j() {
      return !this.resourcePackEntry.isOrderLocked() || !this.resourcePackEntry.isAlwaysEnabled();
   }
   ...
   protected boolean func_195023_m() {
      ...
      return i > 0 && !(list.get(i - 1)).resourcePackEntry.isOrderLocked();
   }

   protected boolean func_195021_n() {
      List<ResourcePackListEntryFound> list = this.getList().getChildren();
      int i = list.indexOf(this);
      return i >= 0 && i < list.size() - 1 && !(list.get(i + 1)).resourcePackEntry.isOrderLocked();
   }
   ...
   public boolean mouseClicked(double p_mouseClicked_1_, double p_mouseClicked_3_, int p_mouseClicked_5_) {
      ...
      if (this.func_195024_j() && d0 <= 32.0D) {
         if (this.func_195025_k()) {
            ...
            PackCompatibility packcompatibility = this.func_195019_f();
            String s2 = this.getEaglerFolderName();
            if (s2 != null) {
               if (InputMappings.isKeyDown(340) || InputMappings.isKeyDown(344)) {
                  this.proceedWithBs(false, packcompatibility, s2);
               } else {
                  this.client.displayGuiScreen(new GuiYesNo((p_210102_1_, p_210102_2_) -> {
                     this.client.displayGuiScreen(this.func_195018_o());
                     this.proceedWithBs(p_210102_1_, packcompatibility, s2);
                  }, I18n.format("eaglercraft.resourcePack.prompt.title", this.getResourcePackName()), I18n.format("eaglercraft.resourcePack.prompt.text"), I18n.format("eaglercraft.resourcePack.prompt.delete"), I18n.format("eaglercraft.resourcePack.prompt.add"), 0));
               }
   ...
   private void proceedWithBs(boolean deleteInstead, PackCompatibility packcompatibility, String folderName) {
      if (!deleteInstead && !packcompatibility.func_198968_a()) {
         String s1 = field_195030_g.getFormattedText();
         String s = packcompatibility.func_198971_c().getFormattedText();
         this.client.displayGuiScreen(new GuiYesNo((p_210102_1_, p_210102_2_) -> {
            this.client.displayGuiScreen(this.func_195018_o());
         ...
         EaglerFolderResourcePack.deleteResourcePack(EaglerFolderResourcePack.RESOURCE_PACKS, folderName);
         this.func_195018_o().refreshResourcePacks();
      } else {
         this.func_195018_o().func_195301_a(this);
      }
   }
   ...
   protected String getEaglerFolderName() {
      String s = this.resourcePackEntry.getName();
      return s.startsWith("folder/") ? s.substring("folder/".length()) : null;
   }
