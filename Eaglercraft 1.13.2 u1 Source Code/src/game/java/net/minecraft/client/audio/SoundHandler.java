...
public class SoundHandler implements ITickable, IResourceManagerReloadListener {
   ...
   public void onResourceManagerReload(IResourceManager resourceManager) {
      ...
      for(ResourceLocation resourcelocation1 : this.soundRegistry.keySet()) {
         if (IRegistry.SOUND_EVENT.get(resourcelocation1) == null) {
   ...
   protected Map<String, SoundList> getSoundMap(InputStream stream) {
      Map<String, SoundList> map = Maps.newHashMap();
      try {
         JsonObject jsonobject = JsonUtils.fromJson(GSON, new InputStreamReader(stream, StandardCharsets.UTF_8), JsonObject.class);
         if (jsonobject != null) {
            for(Entry<String, JsonElement> entry : jsonobject.entrySet()) {
               map.put(entry.getKey(), GSON.fromJson(entry.getValue(), SoundList.class));
            }
         }
      } finally {
