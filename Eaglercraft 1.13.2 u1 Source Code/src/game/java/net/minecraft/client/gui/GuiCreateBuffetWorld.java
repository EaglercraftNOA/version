...
public class GuiCreateBuffetWorld extends GuiScreen {
   private static final List<ResourceLocation> BUFFET_GENERATORS = IRegistry.CHUNK_GENERATOR_TYPE.keySet().stream().filter((p_205307_0_) -> {
      return IRegistry.CHUNK_GENERATOR_TYPE.get(p_205307_0_).isOptionForBuffetWorld();
   ...
   public GuiCreateBuffetWorld(GuiCreateWorld parentIn, NBTTagCompound p_i49701_2_) {
      ...
      for(ResourceLocation resourcelocation : IRegistry.BIOME.keySet()) {
         this.biomeTypes[i] = resourcelocation;
         ++i;
      }

      Arrays.sort(this.biomeTypes, (p_210140_0_, p_210140_1_) -> {
         String s = IRegistry.BIOME.get(p_210140_0_).getDisplayName().getString();
         String s1 = IRegistry.BIOME.get(p_210140_1_).getDisplayName().getString();
   ...
   private NBTTagCompound serialize() {
      ...
      nbttagcompound1.putString("type", IRegistry.BIOME_SOURCE_TYPE.getKey(BiomeProviderType.FIXED).toString());
      ...
      nbttagcompound2.put("biomes", nbttaglist);
      nbttagcompound1.put("options", nbttagcompound2);
      NBTTagCompound nbttagcompound3 = new NBTTagCompound();
      NBTTagCompound nbttagcompound4 = new NBTTagCompound();
      nbttagcompound3.putString("type", BUFFET_GENERATORS.get(this.field_205312_t).toString());
      nbttagcompound4.putString("default_block", "minecraft:stone");
      nbttagcompound4.putString("default_fluid", "minecraft:water");
      nbttagcompound3.put("options", nbttagcompound4);
      nbttagcompound.put("biome_source", nbttagcompound1);
      nbttagcompound.put("chunk_generator", nbttagcompound3);
   ...
   class BiomeList extends GuiSlot {
      ...
      protected void drawSlot(int slotIndex, int xPos, int yPos, int heightIn, int mouseXIn, int mouseYIn, float partialTicks) {
         this.drawString(GuiCreateBuffetWorld.this.fontRenderer, IRegistry.BIOME.get(GuiCreateBuffetWorld.this.biomeTypes[slotIndex]).getDisplayName().getString(), xPos + 5, yPos + 2, 16777215);
      }
   }
}
