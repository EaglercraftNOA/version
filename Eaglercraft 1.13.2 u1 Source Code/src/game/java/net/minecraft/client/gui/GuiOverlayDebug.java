...
public class GuiOverlayDebug extends Gui {
   ...
   public void renderHUDText() {
      ...
      if (this.mc.gameSettings.hudFps) {
         this.fontRenderer.drawStringWithShadow(this.mc.worldRenderer.getDebugInfoShort(), 2.0F, 2.0F, 16777215);
         i += this.fontRenderer.FONT_HEIGHT;
      }
      if (this.mc.gameSettings.hudCoords) {
         this.fontRenderer.drawStringWithShadow(this.mc.worldRenderer.getXYZPosition(), 2.0F, 2.0F + (float)i, 16777215);
      }
      this.drawSingleplayerStats();
   }
   ...
   private int drawSingleplayerStats() {
      ...
      if (SingleplayerServerController.isWorldRunning()) {
         ...
         if (tpsAge < 20000L) {
            int color = tpsAge > 2000L ? 7829367 : 16777215;
            ...
            for(int j = 0, m = strs.size(); j < m; ++j) {
               ...
               if (!StringUtils.isAllEmpty(str)) {
                  l = (int)((float)this.fontRenderer.getStringWidth(str) * (!first ? 0.5F : 1.0F));
                  GlStateManager.pushMatrix();
                  GlStateManager.translatef((float)(this.mc.mainWindow.getScaledWidth() - 2 - l), (float)(i + 2), 0.0F);
                  if (!first) {
                     GlStateManager.scalef(0.5F, 0.5F, 0.5F);
                  }
                  this.fontRenderer.drawStringWithShadow(str, 0.0F, 0.0F, color);
                  GlStateManager.popMatrix();
                  if (color == 16777215) {
                     color = 14737632;
                  }
               }
               i += (int)((float)this.fontRenderer.FONT_HEIGHT * (!first ? 0.5F : 1.0F));
               first = false;
            }
         }
      }
   ...
   protected List<String> call() {
      ...
      if (this.mc.isReducedDebug()) {
         return Lists.newArrayList("Minecraft 1.13.2 (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ")", this.mc.debug, s, this.mc.worldRenderer.getDebugInfoRenders(), this.mc.worldRenderer.getDebugInfoEntities(), "P: " + this.mc.particles.getStatistics() + ". T: " + this.mc.world.getDebugLoadedEntities(), this.mc.world.getProviderName(), "", String.format("Chunk-relative: %d %d %d", blockpos.getX() & 15, blockpos.getY() & 15, blockpos.getZ() & 15));
      } else {
         Entity entity = this.mc.getRenderViewEntity();
   ...
   protected List<String> getDebugInfoRight() {
      ...
      if (this.mc.isReducedDebug()) {
         ...
         if (this.rayTraceBlock != null && this.rayTraceBlock.type == RayTraceResult.Type.BLOCK) {
            ...
            list.add(String.valueOf((Object)IRegistry.BLOCK.getKey(iblockstate.getBlock())));
         ...
         if (this.rayTraceFluid != null && this.rayTraceFluid.type == RayTraceResult.Type.BLOCK) {
            ...
            list.add(String.valueOf((Object)IRegistry.FLUID.getKey(ifluidstate.getFluid())));
         ...
         if (entity != null) {
            ...
            list.add(String.valueOf((Object)IRegistry.ENTITY_TYPE.getKey(entity.getType())));
         }
