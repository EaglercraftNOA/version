...
public class NarratorChatListener implements IChatListener {
      ...
      if (i != 0 && this.narrator.active()) {
         if (i == 1 || i == 2 && chatTypeIn == ChatType.CHAT || i == 3 && chatTypeIn == ChatType.SYSTEM) {
            if (message instanceof TextComponentTranslation && "chat.type.text".equals(((TextComponentTranslation)message).getKey())) {
               this.narrator.say((new TextComponentTranslation("chat.type.text.narrate", ((TextComponentTranslation)message).getFormatArgs())).getString(), false);
            } else {
               this.narrator.say(message.getString(), false);
            }
         }
   ...
   public void announceMode(int p_193641_1_) {
      this.narrator.clear();
      this.narrator.say((new TextComponentTranslation("options.narrator")).getString() + " : " + (new TextComponentTranslation(GameSettings.NARRATOR_MODES[p_193641_1_])).getString(), true);
