...
public class GuiChat extends GuiScreen {
   ...
   protected void initGui() {
      ...
      if (!(this instanceof GuiSleepMP)) {
         this.exitButton = this.addButton(new GuiButton(69, this.width - 100, 3, 97, 20, I18n.hasKey("chat.exit") ? I18n.format("chat.exit") : "Exit Chat") {
            public void onClick(double mouseX, double mouseY) {
               //audrey <3
               GuiChat.this.mc.displayGuiScreen((GuiScreen)null);
            }
         });
      }
   ...
   private void fillNodeUsage(TextFormatting p_195132_1_) {
      ...
      if (!commandcontextbuilder1.getNodes().isEmpty()) {
         ...
         if (this.currentParse.getReader().canRead()) {
            ParsedCommandNode<ISuggestionProvider> parsedcommandnode = commandcontextbuilder1.getNodes().get(commandcontextbuilder1.getNodes().size() - 1);
            commandnode = parsedcommandnode.getNode();
            i = parsedcommandnode.getRange().getEnd() + 1;
         } else if (commandcontextbuilder1.getNodes().size() > 1) {
            ParsedCommandNode<ISuggestionProvider> parsedcommandnode1 = commandcontextbuilder1.getNodes().get(commandcontextbuilder1.getNodes().size() - 2);
            commandnode = parsedcommandnode1.getNode();
            i = parsedcommandnode1.getRange().getEnd() + 1;
         } else {
            if (commandcontextbuilder == commandcontextbuilder1 || commandcontextbuilder1.getNodes().isEmpty()) {
               return;
            }

            ParsedCommandNode<ISuggestionProvider> parsedcommandnode2 = commandcontextbuilder1.getNodes().get(commandcontextbuilder1.getNodes().size() - 1);
            commandnode = parsedcommandnode2.getNode();
            i = parsedcommandnode2.getRange().getEnd() + 1;
         }
