...
public class GuiScreenResourcePacks extends GuiScreen {
   ...
   protected void initGui() {
      ...
      this.addButton(new GuiOptionButton(1, this.width / 2 + 4, this.height - 48, I18n.format("gui.done")) {
         public void onClick(double mouseX, double mouseY) {
            if (GuiScreenResourcePacks.this.changed) {
               ...
               GuiScreenResourcePacks.this.mc.getResourcePackList().setEnabledPacks(list1);
               ...
               for(ResourcePackInfoClient resourcepackinfoclient2 : list1) {
                  if (!resourcepackinfoclient2.isOrderLocked()) {
      ...
      if (!this.changed) {
         ...
         for(ResourcePackInfoClient resourcepackinfoclient1 : Lists.reverse(Lists.newArrayList(resourcepacklist.getEnabledPacks()))) {
   ...
   public void tick() {
      if (EaglerFolderResourcePack.isSupported() && EagRuntime.fileChooserHasResult()) {
         ...
         if (filechooseresult != null) {
            this.mc.loadingScreen.eaglerShow(I18n.format("eaglercraft.resourcePack.load.loading"), filechooseresult.fileName);

            try {
               EaglerFolderResourcePack.importResourcePack(filechooseresult.fileName, EaglerFolderResourcePack.RESOURCE_PACKS, filechooseresult.fileData);
            } catch (IOException ioexception) {
               LOGGER.error("Could not load resource pack: {}", filechooseresult.fileName);
               LOGGER.error(ioexception);
               this.mc.displayGuiScreen(new GuiScreenGenericErrorMessage("eaglercraft.resourcePack.importFailed.1", "eaglercraft.resourcePack.importFailed.2", this.parentScreen));
               return;
            }
