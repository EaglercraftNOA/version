...
public class Font implements AutoCloseable {
   ...
   private static final Random field_212462_d = new net.lax1dude.eaglercraft.v1_8.Random();
   ...
   private boolean glyphsByWidthReady;
   ...
   public void setGlyphProviders(List<IGlyphProvider> glyphProvidersIn) {
      ...
      this.glyphsByWidthReady = false;
      this.glyphProviders.addAll(glyphProvidersIn);
   }

   private void loadGlyphsByWidth() {
      for(char c0 = 0; c0 < '\uffff'; ++c0) {
         for(IGlyphProvider iglyphprovider1 : this.glyphProviders) {
   ...
   public void deleteTextures() {
      for(FontTexture fonttexture : this.textures) {
         this.textureManager.deleteTexture(fonttexture.getTextureLocation());
         fonttexture.close();
      }
   ...
   public TexturedGlyph obfuscate(IGlyph glyph) {
      if (!this.glyphsByWidthReady) {
         this.loadGlyphsByWidth();
      }
