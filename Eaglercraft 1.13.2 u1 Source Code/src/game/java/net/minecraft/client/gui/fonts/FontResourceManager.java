...
public class FontResourceManager implements IResourceManagerReloadListener {
   ...
   public void onResourceManagerReload(IResourceManager resourceManager) {
      ...
      Set<ResourceLocation> set = Sets.newLinkedHashSet(this.fontRenderers.keySet());
      set.addAll(map.keySet());

      for(ResourceLocation resourcelocation2 : set) {
         List<IGlyphProvider> list1 = map.get(resourcelocation2);
         if (list1 == null) {
            list1 = Lists.newArrayList(new DefaultGlyphProvider());
         } else {
            Collections.reverse(list1);
         }

         this.fontRenderers.computeIfAbsent(resourcelocation2, (p_211505_1_) -> {
