...
public class GuiIngame extends Gui {
   ...
   private final Random rand = new net.lax1dude.eaglercraft.v1_8.Random();
   ...
   public void renderGameOverlay(float partialTicks) {
      ...
      if (this.mc.gameSettings.showDebugInfo) {
         ...
         // audrey <3
         this.overlayDebug.renderHUDText();
      }
   ...
   protected void renderPotionEffects() {
      ...
      if (!collection.isEmpty()) {
         ...
         for(PotionEffect potioneffect : Ordering.natural().reverse().sortedCopy(collection)) {
            ...
            if (potion.hasStatusIcon() && potioneffect.isShowIcon()) {
   ...
   private void renderScoreboard(ScoreObjective objective) {
      ...
      for(Score score1 : collection) {
         String s2 = score1.getPlayerName();
   ...
   private int applyTouchHotbarTransformX(int posX, boolean scaled) {
      return scaled ? posX : posX / Math.max(1, (int)this.mc.mainWindow.getGuiScaleFactor());
   }

   private int applyTouchHotbarTransformY(int posY, boolean scaled) {
      return scaled ? posY : this.mc.mainWindow.getScaledHeight() - posY / Math.max(1, (int)this.mc.mainWindow.getGuiScaleFactor()) - 1;
   }

   private int getHotbarSlotTouched(int pointX, int pointY) {
      EntityPlayer entityplayer = this.func_212304_m();
      if (entityplayer == null || pointY < this.mc.mainWindow.getScaledHeight() - 24 || pointY >= this.mc.mainWindow.getScaledHeight()) {
         return -1;
      }
      int center = this.mc.mainWindow.getScaledWidth() / 2;
      int hotbarX = center - 91;
      if (pointX >= hotbarX && pointX < hotbarX + 182) {
         int slot = (pointX - hotbarX) / 20;
      ...
      if (!itemstack.isEmpty()) {
         EnumHandSide enumhandside = entityplayer.getPrimaryHand().opposite();
         if (enumhandside == EnumHandSide.LEFT) {
            if (pointX >= center - 91 - 29 && pointX < center - 91) {
               return 9;
            }
         } else if (pointX >= center + 91 && pointX < center + 91 + 29) {
            return 9;
         }
      }
      return -1;
   ...
   public boolean handleTouchBeginEagler(int uid, int pointX, int pointY) {
      ...
      int slot = getHotbarSlotTouched(pointX, pointY);
      if (slot == -1) {
         return false;
      }
      touchEventUID = uid;
      currentHotbarSlotTouch = slot;
      hotbarSlotTouchStart = Util.milliTime();
      if (slot >= 0 && slot < 9) {
         if (this.mc.player.isSpectator()) {
            this.spectatorGui.onHotbarSelected(slot);
         } else {
            hotbarSlotTouchAlreadySelected = entityplayer.inventory.currentItem == slot;
            entityplayer.inventory.currentItem = slot;
         }
      } else {
   ...
   public boolean handleTouchEndEagler(int uid, int pointX, int pointY) {
      ...
      if (hotbarSlotTouchStart != -1L && currentHotbarSlotTouch >= 0 && currentHotbarSlotTouch < 9 && !this.mc.player.isSpectator()) {
         if (Util.milliTime() - hotbarSlotTouchStart < 350L && hotbarSlotTouchAlreadySelected) {
   ...
   public void updateTouchEagler(boolean screenTouched) {
      if (screenTouched) {
         ...
         for(int i = 0; i < pointCount; ++i) {
            ...
            int pointX = applyTouchHotbarTransformX(Touch.touchPointX(i), false);
            int pointY = applyTouchHotbarTransformY(this.mc.mainWindow.getHeight() - Touch.touchPointY(i) - 1, false);
            if (uid == touchEventUID && hotbarSlotTouchStart != -1L) {
               int slot = getHotbarSlotTouched(pointX, pointY);
               if (slot != currentHotbarSlotTouch && slot >= 0 && slot < 9 && !this.mc.player.isSpectator()) {
                  currentHotbarSlotTouch = slot;
                  hotbarSlotTouchAlreadySelected = this.mc.player.inventory.currentItem == slot;
      ...
      } else if (touchEventUID != -1) {
         handleTouchEndEagler(touchEventUID, 0, 0);
      }
   }

   public boolean isTouchOverlapEagler(int uid, int tx, int ty) {
      tx = applyTouchHotbarTransformX(tx, false);
      ty = applyTouchHotbarTransformY(this.mc.mainWindow.getHeight() - ty - 1, false);
      return getHotbarSlotTouched(tx, ty) != -1;
   }
