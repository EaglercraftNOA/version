...
public class ParticleBubbleColumnUp extends Particle {
   protected ParticleBubbleColumnUp(World p_i48833_1_, double p_i48833_2_, double p_i48833_4_, double p_i48833_6_, double p_i48833_8_, double p_i48833_10_, double p_i48833_12_) {
      ...
      this.motionX = p_i48833_8_ * (double)0.2F + (this.rand.nextDouble() * 2.0D - 1.0D) * (double)0.02F;
      this.motionY = p_i48833_10_ * (double)0.2F + (this.rand.nextDouble() * 2.0D - 1.0D) * (double)0.02F;
      this.motionZ = p_i48833_12_ * (double)0.2F + (this.rand.nextDouble() * 2.0D - 1.0D) * (double)0.02F;
      this.maxAge = (int)(40.0D / (this.rand.nextDouble() * 0.8D + 0.2D));
   }
   ...
   public void tick() {
      ...
      if (!this.world.getFluidState(getBlockPos(this.posX, this.posY, this.posZ)).isTagged(FluidTags.WATER)) {
         this.setExpired();
      }
