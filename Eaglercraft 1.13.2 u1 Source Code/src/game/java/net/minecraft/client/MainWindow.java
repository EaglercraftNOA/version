...
public final class MainWindow implements AutoCloseable {
   ...
   public MainWindow(Minecraft mcIn, VirtualScreen virtualScreenIn, GameConfiguration.DisplayInformation displayInfoIn, String fullscreenResolution) {
      ...
      this.handle = PlatformRuntime.getWindowHandle();
   ...
   private void loadIcon() {
      try (
         ...
         buffer.get(0).width(intbuffer.get(0));
         buffer.get(0).height(intbuffer1.get(0));
         buffer.get(0).pixels(bytebuffer);
         ByteBuffer bytebuffer1 = this.func_198111_a(inputstream1, intbuffer, intbuffer1, intbuffer2);
         if (bytebuffer1 == null) {
            throw new IllegalStateException("Could not load icon: " + STBImage.stbi_failure_reason());
         }

         buffer.get(1).width(intbuffer.get(0));
         buffer.get(1).height(intbuffer1.get(0));
         buffer.get(1).pixels(bytebuffer1);
   ...
   public void update(boolean limitFps) {
      ...
      if (limitFps && this.isFramerateLimitBelowMax()) {
