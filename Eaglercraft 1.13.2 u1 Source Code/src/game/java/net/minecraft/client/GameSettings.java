...
public class GameSettings {
   ...
   public boolean hasCreatedDemoWorld = false;
   public int relayTimeout = 5;
   ...
   public GameSettings(Minecraft mcIn, File mcDataDir) {
      ...
      if (mcIn.isJava64bit() && EagRuntime.maxMemory() >= 1000000000L) {
   ...
   public void loadOptions() {
      byte[] data = EagRuntime.getStorage("g");
      if (data == null) {
         return;
      }
      loadOptions(data);
   ...
   public void loadOptions(byte[] data) {
      try {
         ...
         try(java.io.BufferedReader rd = new java.io.BufferedReader(new InputStreamReader(EaglerZLIB.newGZIPInputStream(new EaglerInputStream(data)), StandardCharsets.UTF_8))) {
            String ln;
         ...
         for(String s1 : nbttagcompound.keySet()) {
            ...
            try {
               ...
               if ("resourcePacks".equals(s1)) {
                  ...
                  for(JsonElement packElement : new JsonParser().parse(s2).getAsJsonArray()) {
                     this.resourcePacks.add(packElement.getAsString());
                  }
               }
               ...
               if ("incompatibleResourcePacks".equals(s1)) {
                  ...
                  for(JsonElement packElement : new JsonParser().parse(s2).getAsJsonArray()) {
                     this.incompatibleResourcePacks.add(packElement.getAsString());
                  }
               }
               ...
               if ("enableUpdateSvc".equals(s1)) {
                  this.enableUpdateSvc = "true".equals(s2);
               }
               ...
               if ("hasCreatedDemoWorld".equals(s1)) {
                  this.hasCreatedDemoWorld = "true".equals(s2);
               }

               if ("relayTimeout".equals(s1)) {
                  this.relayTimeout = Integer.parseInt(s2);
               }
   ...
   public byte[] writeOptions() {
      ...
      try {
         printwriter = new PrintWriter(new OutputStreamWriter(EaglerZLIB.newGZIPOutputStream(bao)));
   ...
   public void fillResourcePackList(ResourcePackList<ResourcePackInfoClient> resourcePackListIn) {
      ...
      while(iterator.hasNext()) {
         ...
         if (resourcepackinfoclient == null && !s.startsWith("folder/")) {
            resourcepackinfoclient = resourcePackListIn.getPackInfo("folder/" + s);
         }
