...
public class CraftPlanksStep implements ITutorialStep {
      ...
      if (this.tutorial.getGameType() != GameType.SURVIVAL) {
         ...
         if (this.timeWaiting == 1) {
            ...
            if (entityplayersp != null) {
               ...
               if (hasCrafted(entityplayersp, ItemTags.PLANKS)) {
                  this.tutorial.setStep(TutorialSteps.NONE);
                  return;
               }
            }
   ...
   public static boolean hasCrafted(EntityPlayerSP player, Tag<Item> itemsIn) {
      for(Item item : itemsIn.getAllElements()) {
         if (player.getStats().getValue(StatList.ITEM_CRAFTED.get(item)) > 0) {
            return true;
         }
      }
