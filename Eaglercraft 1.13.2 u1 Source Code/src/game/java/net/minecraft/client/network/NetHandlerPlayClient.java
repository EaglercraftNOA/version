...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
   ...
   private ServerTextureCache textureCache = null;
   ...
   public NetHandlerPlayClient(Minecraft mcIn, GuiScreen p_i46300_2_, EaglercraftNetworkManager networkManagerIn, GameProfile profileIn, GamePluginMessageProtocol eaglerProtocol) {
      this(mcIn, p_i46300_2_, (NetworkManager)networkManagerIn, profileIn);
      ClientMessageHandler handler = ClientMessageHandler.createClientHandler(eaglerProtocol.ver, this);
      if(eaglerProtocol.ver >= 5) {
         this.eaglerMessageController = new InjectedMessageController(eaglerProtocol, handler, GamePluginMessageConstants.CLIENT_TO_SERVER, networkManagerIn::injectRawFrame);
         networkManagerIn.setInjectedMessageController((InjectedMessageController)this.eaglerMessageController);
      }else {
         this.eaglerMessageController = new LegacyMessageController(eaglerProtocol, handler, GamePluginMessageConstants.CLIENT_TO_SERVER, (ch, msg) -> this.sendPacket(new CPacketCustomPayload(encodeEaglerLegacyChannel(ch), msg)));
      }
      this.textureCache = ServerTextureCache.create(this, mcIn.getTextureManager());
   }

   public static ResourceLocation encodeEaglerLegacyChannel(String channel) {
      StringBuilder path = new StringBuilder(channel.length() * 2);
      for(int i = 0, l = channel.length(); i < l; ++i) {
         int c = channel.charAt(i);
         path.append(Character.forDigit((c >>> 4) & 15, 16));
         path.append(Character.forDigit(c & 15, 16));
      }

      return new ResourceLocation("eagler_legacy", path.toString());
   }

   public static String decodeEaglerLegacyChannel(ResourceLocation channel) {
      if(!"eagler_legacy".equals(channel.getNamespace())) {
         return null;
      }
      ...
      StringBuilder ret = new StringBuilder(l >> 1);
      for(int i = 0; i < l; i += 2) {
         int hi = Character.digit(path.charAt(i), 16);
         int lo = Character.digit(path.charAt(i + 1), 16);
         if(hi == -1 || lo == -1) {
            return null;
         }

         ret.append((char)((hi << 4) | lo));
      }
   ...
   public boolean webViewSendHandler(GameMessagePacket pkt) {
      ...
      if (this.client.player == null || this.client.player.connection != this) {
   ...
   public void handleJoinGame(SPacketJoinGame packetIn) {
      ...
      if (this.client.isSingleplayer()) {
         this.client.ingameGUI.getChatGUI().printChatMessage(new TextComponentString(TextFormatting.GOLD + "[EaglercraftX] " + TextFormatting.AQUA + "Use /eagskull to create custom skulls"));
      }
   }
         ...
         if (packetIn.getData() > 0) {
            if (packetIn.getType() == 60 || packetIn.getType() == 91 || packetIn.getType() == 94) {
               ...
               if (entity3 instanceof EntityLivingBase && entity instanceof EntityArrow) {
                  ...
                  entityarrow.setShooter(entity3);
   ...
   public void handleSpawnPainting(SPacketSpawnPainting packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      EntityPainting entitypainting = new EntityPainting(this.world, packetIn.getPosition(), packetIn.getFacing(), packetIn.getType());
   ...
   public void handleRespawn(SPacketRespawn packetIn) {
      ...
      if (dimensiontype != this.client.player.dimension) {
         ...
         this.world = new WorldClient(this, new WorldSettings(0L, packetIn.getGameType(), false, this.client.world.getWorldInfo().isHardcore(), packetIn.getWorldType()), packetIn.getDimension(), packetIn.getDifficulty(), this.client.profiler);
   ...
   public void handleChangeGameState(SPacketChangeGameState packetIn) {
      ...
      if (i == 1) {
         ...
         this.world.addParticle(Particles.ELDER_GUARDIAN, entityplayer.posX, entityplayer.posY, entityplayer.posZ, 0.0D, 0.0D, 0.0D);
   ...
   public void handleMaps(SPacketMaps packetIn) {
      ...
      if (mapdata == null) {
         ...
         this.client.world.setSavedData(DimensionType.OVERWORLD, s, mapdata);
      }
   ...
   public void handleEntityEffect(SPacketEntityEffect packetIn) {
      ...
      if (entity instanceof EntityLivingBase) {
         ...
         if (potion != null) {
            PotionEffect potioneffect = new PotionEffect(potion, packetIn.getDuration(), packetIn.getAmplifier(), packetIn.getIsAmbient(), packetIn.doesShowParticles(), packetIn.shouldShowIcon());
   ...
   public void handlePlayerListItem(SPacketPlayerListItem packetIn) {
      ...
      for(SPacketPlayerListItem.AddPlayerData spacketplayerlistitem$addplayerdata : packetIn.getEntries()) {
         if (packetIn.getAction() == SPacketPlayerListItem.Action.REMOVE_PLAYER) {
            UUID uuid = spacketplayerlistitem$addplayerdata.getProfile().getId();
            this.playerInfoMap.remove(uuid);
            EaglercraftUUID eagleruuid = new EaglercraftUUID(EaglerUUIDHelper.getMostSignificantBits(uuid), EaglerUUIDHelper.getLeastSignificantBits(uuid));
            if (this.textureCache != null) {
               this.textureCache.evictPlayer(eagleruuid);
            }

            ClientUUIDLoadingCache.evict(eagleruuid);
         } else {
   ...
   public void handleCustomSound(SPacketCustomSound packetIn) {
      PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.client);
      this.client.getSoundHandler().play(new SimpleSound(packetIn.getSoundName(), packetIn.getCategory(), packetIn.getVolume(), packetIn.getPitch(), false, 0, ISound.AttenuationType.LINEAR, (float)packetIn.getX(), (float)packetIn.getY(), (float)packetIn.getZ()));
   }
   ...
   public void handleCustomPayload(SPacketCustomPayload packetIn) {
      ...
      try {
         ...
         } else if (this.eaglerMessageController instanceof LegacyMessageController) {
            String s = decodeEaglerLegacyChannel(resourcelocation);
            if (s != null) {
               try {
                  ((LegacyMessageController)this.eaglerMessageController).handlePacket(s, packetbuffer);
               } catch (IOException ioexception1) {
                  LOGGER.error("Couldn't read \"{}\" packet as an eaglercraft plugin message!", (Object)s);
   ...
   public void handlePlaceGhostRecipe(SPacketPlaceGhostRecipe packetIn) {
      ...
      if (container.windowId == packetIn.getWindowId() && container.getCanCraft(this.client.player)) {
         IRecipe irecipe = this.recipeManager.getRecipe(packetIn.getRecipeId());
         if (irecipe != null) {
            if (this.client.currentScreen instanceof IRecipeShownListener) {
