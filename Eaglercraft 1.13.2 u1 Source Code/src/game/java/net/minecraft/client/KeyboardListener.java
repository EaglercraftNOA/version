...
public class KeyboardListener {
   ...
   private final ByteBuffer nativeBuffer = ByteBuffer.allocateDirect(1024);
   ...
   private boolean processKeyF3(int key) {
      if (this.debugCrashKeyPressTime > 0L && this.debugCrashKeyPressTime < Util.milliTime() - 100L) {
         ...
         case 73:
            if (!this.mc.player.hasReducedDebug()) {
               this.copyHoveredObject(this.mc.player.hasPermissionLevel(2), !GuiScreen.isShiftKeyDown());
            }
   ...
   private void copyHoveredObject(boolean privileged, boolean askServer) {
      if (this.mc.objectMouseOver != null) {
         ...
         case ENTITY:
            ...
            ResourceLocation resourcelocation = IRegistry.ENTITY_TYPE.getKey(entity.getType());
   ...
   private void setClipboardStringNative(ByteBuffer p_211559_1_, String p_211559_2_) {
   ...
   public void setClipboardString(String string) {
      ...
      if (i < this.nativeBuffer.capacity()) {
         this.setClipboardStringNative(this.nativeBuffer, string);
         this.nativeBuffer.clear();
      } else {
         ByteBuffer bytebuffer = ByteBuffer.allocateDirect(i);
         this.setClipboardStringNative(bytebuffer, string);
      }
