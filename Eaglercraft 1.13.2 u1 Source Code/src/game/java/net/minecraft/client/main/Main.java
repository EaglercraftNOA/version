...
public class Main {
   ...
   public static void appMain() {
      ...
      if (username == null || username.isEmpty()) {
         username = "Player" + Util.milliTime() % 1000L;
         EaglerProfile.setName(username);
      }
      File file1 = new File(".");
      File file4 = new File(file1, "resources/");
      VanillaPack.baseClass = Main.class;
      File file2 = new File(file4, "assets/");
      File file3 = new File(file1, "resourcepacks/");
      Session session = new Session(username, EaglerUUIDHelper.nameUUIDFromBytes(("OfflinePlayer:" + username).getBytes(StandardCharsets.UTF_8)).toString(), "0", "legacy");
      GameConfiguration gameconfiguration = new GameConfiguration(new GameConfiguration.UserInformation(session, new PropertyMap(), new PropertyMap(), null), new GameConfiguration.DisplayInformation(854, 480, Optional.empty(), Optional.empty(), false), new GameConfiguration.FolderInformation(file1, file3, file2, null), new GameConfiguration.GameInformation(false, "1.13.2", "release"), new GameConfiguration.ServerInformation(null, 25565));
