...
public class WorldRenderer implements IWorldEventListener, AutoCloseable, IResourceManagerReloadListener {
   ...
   public static final EnumFacing[] FACINGS = EnumFacing._VALUES;
   private final Minecraft mc;
   private final TextureManager textureManager;
   private final RenderManager renderManager;
   private WorldClient world;
   private Set<RenderChunk> chunksToUpdate = Sets.newLinkedHashSet();
   private Set<RenderChunk> chunksToUpdateTemporary = Sets.newLinkedHashSet();
   private final List<WorldRenderer.ContainerLocalRenderInformation> renderInfos = Lists.newArrayList();
   private final List<WorldRenderer.ContainerLocalRenderInformation> renderInfoPool = Lists.newArrayList();
   private final Queue<WorldRenderer.ContainerLocalRenderInformation> renderInfoQueue = Queues.newArrayDeque();
   private int renderInfoPoolIndex;
   private final List<Entity> entityOutlineRenderList = Lists.newArrayList();
   private final List<Entity> entityMultipassRenderList = Lists.newArrayList();
   private final BlockPos.MutableBlockPos terrainCullPos = new BlockPos.MutableBlockPos();
   private final BlockPos.MutableBlockPos terrainViewPos = new BlockPos.MutableBlockPos();
   private final BlockPos.MutableBlockPos terrainChunkPos = new BlockPos.MutableBlockPos();
   ...
   private int countChunksRendered;
   ...
   private static final IRenderChunkFactory renderChunkFactoryVbo = new IRenderChunkFactory() {
      public RenderChunk create(World worldIn, WorldRenderer renderGlobalIn) {
         return new RenderChunk(worldIn, renderGlobalIn);
      }
   };
   private static final IRenderChunkFactory renderChunkFactoryList = new IRenderChunkFactory() {
      public RenderChunk create(World worldIn, WorldRenderer renderGlobalIn) {
         return new ListedRenderChunk(worldIn, renderGlobalIn);
      }
   };
   ...
   public WorldRenderer(Minecraft mcIn) {
      ...
      if (this.vboEnabled) {
         ...
         this.renderChunkFactory = renderChunkFactoryVbo;
      } else {
         this.renderContainer = new RenderList();
         this.renderChunkFactory = renderChunkFactoryList;
      }

      this.vertexBufferFormat = VertexFormat.POSITION;
      this.generateStars();
      this.generateSky();
      this.generateSky2();
   }
   ...
   private void renderStars(BufferBuilder bufferBuilderIn) {
      Random random = new net.lax1dude.eaglercraft.v1_8.Random(10842L);
      bufferBuilderIn.begin(7, DefaultVertexFormats.POSITION);
   ...
   public void setWorldAndLoadRenderers(@Nullable WorldClient worldClientIn) {
      ...
      if (worldClientIn != null) {
         ...
         this.chunksToUpdateTemporary.clear();
         this.renderInfos.clear();
         this.renderInfoQueue.clear();
         this.renderInfoPool.clear();
         this.renderInfoPoolIndex = 0;
   ...
   public void renderEntities(Entity renderViewEntity, ICamera camera, float partialTicks) {
      if (this.renderEntitiesStartupCounter > 0) {
         ...
         List<Entity> list = this.entityOutlineRenderList;
         List<Entity> list1 = this.entityMultipassRenderList;
         list.clear();
         list1.clear();

         try (BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain()) {
            for(int k = 0, l = this.renderInfos.size(); k < l; ++k) {
               WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation = this.renderInfos.get(k);
         ...
         if (!list1.isEmpty()) {
            for(int k = 0, l = list1.size(); k < l; ++k) {
               this.renderManager.renderMultipass(list1.get(k), partialTicks);
            }
         }
         ...
         for(int k = 0, l = this.renderInfos.size(); k < l; ++k) {
            WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation1 = this.renderInfos.get(k);
            List<TileEntity> list2 = worldrenderer$containerlocalrenderinformation1.renderChunk.getCompiledChunk().getTileEntities();
            if (!list2.isEmpty()) {
               for(int i1 = 0, j1 = list2.size(); i1 < j1; ++i1) {
                  TileEntityRendererDispatcher.instance.render(list2.get(i1), partialTicks, -1);
               }
            }
         }
   ...
   public String getDebugInfoShort() {
      int i = this.viewFrustum.renderChunks.length;
      return "" + Minecraft.getDebugFPS() + "fps | C: " + this.countChunksRendered + "/" + i + ", E: " + this.countEntitiesRendered + "+" + this.countTileEntitiesRendered + ", " + (this.renderDispatcher == null ? "null" : this.renderDispatcher.getDebugInfo());
   }

   public String getXYZPosition() {
      Entity entity = this.mc.player;
      return "x: " + (int)entity.posX + ", y: " + (int)entity.posY + ", z: " + (int)entity.posZ;
   }

   protected int getRenderedChunks() {
      return this.countChunksRendered;
   }
   ...
   public void setupTerrain(Entity entityIn, float partialTicks, ICamera camera, int frameCount, boolean playerSpectator) {
      ...
      BlockPos.MutableBlockPos blockpos1 = this.terrainViewPos.setPos(d3, d4 + (double)entityIn.getEyeHeight(), d5);
      RenderChunk renderchunk = this.viewFrustum.getRenderChunk(blockpos1);
      BlockPos.MutableBlockPos blockpos = this.terrainChunkPos.setPos(MathHelper.floor(d3 / 16.0D) * 16, MathHelper.floor(d4 / 16.0D) * 16, MathHelper.floor(d5 / 16.0D) * 16);
      float f = entityIn.getPitch(partialTicks);
      float f1 = entityIn.getYaw(partialTicks);
		this.displayListEntitiesDirty = this.displayListEntitiesDirty || entityIn.posX != this.lastViewEntityX || entityIn.posY != this.lastViewEntityY || entityIn.posZ != this.lastViewEntityZ || (double)f != this.lastViewEntityPitch || (double)f1 != this.lastViewEntityYaw;
      ...
      if (!flag && this.displayListEntitiesDirty) {
         ...
         this.renderInfoQueue.clear();
         this.renderInfoPoolIndex = 0;
         Queue<WorldRenderer.ContainerLocalRenderInformation> queue = this.renderInfoQueue;
         ...
         if (renderchunk != null) {
            ...
            queue.add(this.getContainerLocalRenderInformation(renderchunk, (EnumFacing)null, 0));
         } else {
            ...
            for(int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
               for(int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
                  RenderChunk renderchunk1 = this.viewFrustum.getRenderChunk(this.terrainCullPos.setPos((j << 4) + 8, i, (k << 4) + 8));
                  if (renderchunk1 != null && camera.isBoundingBoxInFrustum(renderchunk1.boundingBox)) {
                     renderchunk1.setFrameIndex(frameCount);
                     queue.add(this.getContainerLocalRenderInformation(renderchunk1, (EnumFacing)null, 0));
                  }
               }
            }
         }
         ...
         while(!queue.isEmpty()) {
            ...
            for(EnumFacing enumfacing1 : FACINGS) {
               ...
               if ((!flag1 || !worldrenderer$containerlocalrenderinformation1.hasDirection(enumfacing1.getOpposite())) && (!flag1 || enumfacing2 == null || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing1)) && renderchunk2 != null && renderchunk2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk2.boundingBox)) {
                  WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation = this.getContainerLocalRenderInformation(renderchunk2, enumfacing1, worldrenderer$containerlocalrenderinformation1.counter + 1);
      ...
      this.chunksToUpdate = this.chunksToUpdateTemporary;
      this.chunksToUpdate.clear();
      int i2 = 0;
      int j2 = 0;

      for(int k2 = 0, l2 = this.renderInfos.size(); k2 < l2; ++k2) {
         WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation2 = this.renderInfos.get(k2);
         RenderChunk renderchunk4 = worldrenderer$containerlocalrenderinformation2.renderChunk;
         CompiledChunk compiledchunk = renderchunk4.compiledChunk;
         if (compiledchunk != CompiledChunk.DUMMY && !compiledchunk.isEmpty()) {
            ++i2;
            j2 += compiledchunk.getTileEntities().size();
         }

			if (renderchunk4.needsUpdate() || set.contains(renderchunk4)) {
				BlockPos blockpos2 = renderchunk4.getPosition();
				double d6 = (double)(blockpos2.getX() + 8 - blockpos1.getX());
				double d7 = (double)(blockpos2.getY() + 8 - blockpos1.getY());
				double d8 = (double)(blockpos2.getZ() + 8 - blockpos1.getZ());
				boolean flag3 = d6 * d6 + d7 * d7 + d8 * d8 < 768.0D;
      ...
      this.chunksToUpdateTemporary = set;
      this.countChunksRendered = i2;
      this.countTileEntitiesRendered = j2;
      this.mc.profiler.endSection();
   }

   private WorldRenderer.ContainerLocalRenderInformation getContainerLocalRenderInformation(RenderChunk renderChunkIn, @Nullable EnumFacing facingIn, int counterIn) {
      if (this.renderInfoPoolIndex >= this.renderInfoPool.size()) {
         this.renderInfoPool.add(new WorldRenderer.ContainerLocalRenderInformation(renderChunkIn, facingIn, counterIn));
      }

      WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation = this.renderInfoPool.get(this.renderInfoPoolIndex++);
      worldrenderer$containerlocalrenderinformation.set(renderChunkIn, facingIn, counterIn);
      return worldrenderer$containerlocalrenderinformation;
   }
   ...
   private RenderChunk getRenderChunkOffset(BlockPos playerPos, RenderChunk renderChunkBase, EnumFacing facing) {
      ...
      if (MathHelper.abs(playerPos.getX() - blockpos.getX()) > this.renderDistanceChunks * 16) {
         ...
         return MathHelper.abs(playerPos.getZ() - blockpos.getZ()) > this.renderDistanceChunks * 16 ? null : renderChunkBase.getRenderChunkOffset16(this.viewFrustum, facing);
      } else {
         return null;
      }
   }
   ...
   protected EnumFacing getViewFacing(Entity entityIn, double partialTicks) {
      ...
      return EnumFacing.getFacingFromVector(f3 * f4, f5, f2 * f4);
   }
   ...
   public int renderBlockLayer(BlockRenderLayer blockLayerIn, double partialTicks, Entity entityIn) {
      ...
      if (blockLayerIn == BlockRenderLayer.TRANSLUCENT) {
         ...
         if (d0 * d0 + d1 * d1 + d2 * d2 > 1.0D) {
            ...
            for(int l1 = 0, i2 = this.renderInfos.size(); l1 < i2; ++l1) {
               WorldRenderer.ContainerLocalRenderInformation worldrenderer$containerlocalrenderinformation = this.renderInfos.get(l1);
               if (worldrenderer$containerlocalrenderinformation.renderChunk.compiledChunk.isLayerStarted(blockLayerIn)) {
   ...
   public void drawSelectionBox(EntityPlayer player, RayTraceResult movingObjectPositionIn, int execute, float partialTicks) {
      if (execute == 0 && movingObjectPositionIn.type == RayTraceResult.Type.BLOCK) {
         ...
         IBlockState iblockstate = this.world.getBlockState(blockpos);
         if (!iblockstate.isAir() && this.world.getWorldBorder().contains(blockpos)) {
            GlStateManager.enableBlend();
            GlStateManager.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
   ...
   public static void drawShape(VoxelShape voxelShapeIn, double xIn, double yIn, double zIn, float red, float green, float blue, float alpha) {
      ...
      voxelShapeIn.forEachEdge(new DrawShapeLineConsumer(bufferbuilder, xIn, yIn, zIn, red, green, blue, alpha));
      tessellator.draw();
   }

   private static class DrawShapeLineConsumer implements VoxelShapes.LineConsumer {
      ...
      private DrawShapeLineConsumer(BufferBuilder bufferIn, double xIn, double yIn, double zIn, float redIn, float greenIn, float blueIn, float alphaIn) {
      ...
      public void consume(double x1, double y1, double z1, double x2, double y2, double z2) {
         this.buffer.pos(x1 + this.x, y1 + this.y, z1 + this.z).color(this.red, this.green, this.blue, this.alpha).endVertex();
         this.buffer.pos(x2 + this.x, y2 + this.y, z2 + this.z).color(this.red, this.green, this.blue, this.alpha).endVertex();
      }
   }
   ...
   class ContainerLocalRenderInformation {
      ...
      private ContainerLocalRenderInformation(RenderChunk renderChunkIn, @Nullable EnumFacing facingIn, int counterIn) {
         this.set(renderChunkIn, facingIn, counterIn);
      }

      private void set(RenderChunk renderChunkIn, @Nullable EnumFacing facingIn, int counterIn) {
         this.renderChunk = renderChunkIn;
         this.facing = facingIn;
         this.counter = counterIn;
         this.setFacing = facingIn == null ? 0 : (byte)(1 << facingIn.ordinal());
      }
