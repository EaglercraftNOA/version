...
public class VisGraph {
   ...
   private static final EnumFacing[] DIRECTIONS = EnumFacing._VALUES;
   ...
   private static final int[] INDEX_OF_EDGES = makeIndexOfEdges();

   private static int[] makeIndexOfEdges() {
      int[] indexes = new int[1352];
      int i = 0;
      int j = 15;
      int k = 0;
   ...
   public void setOpaqueCube(BlockPos pos) {
      this.opaqueBlocks.set(getIndex(pos), true);
      --this.empty;
   }
   ...
   private Set<EnumFacing> floodFill(int pos) {
      ...
      while(i < j) {
         ...
         for(EnumFacing enumfacing : DIRECTIONS) {
            int l = this.getNeighborIndexAtFace(k, enumfacing);
            if (l >= 0 && !bitset.get(l)) {
               bitset.set(l, true);
               aint[j++] = l;
            }
