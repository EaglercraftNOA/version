   ...
   public void doRender(EntityFishHook entity, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      if (entityplayer != null && !this.renderOutlines) {
         ...
         if ((this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) && entityplayer == Minecraft.getInstance().player) {
            ...
            d8 = d8 / 100.0D;
            double d15 = (double)k * -0.36D * d8;
            double d16 = -0.045D * d8;
            double d17 = 0.4D;
            float f11 = -(entityplayer.prevRotationPitch + (entityplayer.rotationPitch - entityplayer.prevRotationPitch) * partialTicks) * ((float)Math.PI / 180F);
            float f12 = MathHelper.cos(f11);
            float f13 = MathHelper.sin(f11);
            double d18 = d16 * (double)f12 + d17 * (double)f13;
            double d19 = d17 * (double)f12 - d16 * (double)f13;
            float f14 = -(entityplayer.prevRotationYaw + (entityplayer.rotationYaw - entityplayer.prevRotationYaw) * partialTicks) * ((float)Math.PI / 180F);
            float f15 = MathHelper.cos(f14);
            float f16 = MathHelper.sin(f14);
            double d20 = d15 * (double)f15 + d19 * (double)f16;
            double d21 = d19 * (double)f15 - d15 * (double)f16;
            f15 = MathHelper.cos(f8 * 0.5F);
            f16 = MathHelper.sin(f8 * 0.5F);
            d15 = d20 * (double)f15 + d21 * (double)f16;
            d17 = d21 * (double)f15 - d20 * (double)f16;
            f12 = MathHelper.cos(-f8 * 0.7F);
            f13 = MathHelper.sin(-f8 * 0.7F);
            d16 = d18 * (double)f12 + d17 * (double)f13;
            d17 = d17 * (double)f12 - d18 * (double)f13;
            d4 = entityplayer.prevPosX + (entityplayer.posX - entityplayer.prevPosX) * (double)partialTicks + d15;
            d5 = entityplayer.prevPosY + (entityplayer.posY - entityplayer.prevPosY) * (double)partialTicks + d16;
            d6 = entityplayer.prevPosZ + (entityplayer.posZ - entityplayer.prevPosZ) * (double)partialTicks + d17;
