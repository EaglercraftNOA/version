...
public class BlockModelRenderer {
   ...
   private static final Long2IntLinkedOpenHashMap cacheCombinedLight = makeLightCache();
   private static boolean cacheEnabled;
   private static final EnumFacing[] facings = EnumFacing._VALUES;
   private final BlockModelRenderer.RenderContext renderContext = new RenderContext();
   private final Random brightnessRandom = new net.lax1dude.eaglercraft.v1_8.Random();

   private static Long2IntLinkedOpenHashMap makeLightCache() {
      Long2IntLinkedOpenHashMap map = new Long2IntLinkedOpenHashMap(256);
   ...
   public boolean renderModelSmooth(IWorldReader worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, Random randomIn, long rand) {
      boolean flag = false;
      BlockModelRenderer.RenderContext blockmodelrenderer$rendercontext = this.renderContext;
      float[] afloat = blockmodelrenderer$rendercontext.quadBounds;
      BitSet bitset = blockmodelrenderer$rendercontext.boundsFlags;
      BlockModelRenderer.AmbientOcclusionFace blockmodelrenderer$ambientocclusionface = blockmodelrenderer$rendercontext.ambientOcclusionFace;
   ...
   public boolean renderModelFlat(IWorldReader worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, Random randomIn, long rand) {
      ...
      BlockModelRenderer.RenderContext blockmodelrenderer$rendercontext = this.renderContext;
      BitSet bitset = blockmodelrenderer$rendercontext.boundsFlags;
      BlockPos.MutableBlockPos blockpos$mutableblockpos = blockmodelrenderer$rendercontext.blockPos;

      for(EnumFacing enumfacing : facings) {
         randomIn.setSeed(rand);
         List<BakedQuad> list = modelIn.getQuads(stateIn, enumfacing, randomIn);
         if (!list.isEmpty() && (!checkSides || Block.shouldSideBeRendered(stateIn, worldIn, posIn, enumfacing))) {
            int i = stateIn.getPackedLightmapCoords(worldIn, blockpos$mutableblockpos.setPos(posIn).move(enumfacing));
            this.renderQuadsFlat(worldIn, stateIn, posIn, i, false, buffer, list, bitset, blockpos$mutableblockpos);
            flag = true;
         }
      }
      ...
      List<BakedQuad> list1 = modelIn.getQuads(stateIn, (EnumFacing)null, randomIn);
      if (!list1.isEmpty()) {
         this.renderQuadsFlat(worldIn, stateIn, posIn, -1, true, buffer, list1, bitset, blockpos$mutableblockpos);
         flag = true;
      }
   ...
   private void renderQuadsFlat(IWorldReader blockAccessIn, IBlockState stateIn, BlockPos posIn, int brightnessIn, boolean ownBrightness, BufferBuilder buffer, List<BakedQuad> list, BitSet bitSet, BlockPos.MutableBlockPos mutablePos) {
      Vec3d vec3d = stateIn.getOffset(blockAccessIn, posIn);
      double d0 = (double)posIn.getX() + vec3d.x;
      double d1 = (double)posIn.getY() + vec3d.y;
      double d2 = (double)posIn.getZ() + vec3d.z;
      ...
      for(int j = list.size(); i < j; ++i) {
         ...
         if (ownBrightness) {
            ...
            BlockPos blockpos = bitSet.get(0) ? mutablePos.setPos(posIn).move(bakedquad.getFace()) : posIn;
   ...
   public void renderModelBrightnessColor(@Nullable IBlockState state, IBakedModel modelIn, float brightness, float red, float green, float blue) {
      Random random = this.brightnessRandom;
      long i = 42L;
   ...
   class RenderContext {
      ...
      final BlockModelRenderer.AmbientOcclusionFace ambientOcclusionFace = new BlockModelRenderer.AmbientOcclusionFace();
   ...
   private static int getPackedLightmapCoords(IBlockState blockStateIn, IWorldReader worldIn, BlockPos posIn) {
      ...
      if (cacheEnabled) {
         if (cacheCombinedLight.size() >= 256) {
            cacheCombinedLight.removeFirstInt();
         }
         cacheCombinedLight.put(key, j);
      }
   ...
   class AmbientOcclusionFace {
      ...
      public void updateVertexBrightness(IWorldReader worldIn, IBlockState state, BlockPos centerPos, EnumFacing direction, float[] faceShape, BitSet shapeState) {
         BlockPos blockpos = shapeState.get(0) ? this.baseBlockPos.setPos(centerPos).move(direction) : centerPos;
         ...
         BlockPos.MutableBlockPos blockpos$mutableblockpos = this.blockPos;
   ...
   public static enum EnumNeighborInfo {
      ...
      private static final BlockModelRenderer.EnumNeighborInfo[] VALUES = makeEnumNeighborInfoValues();

      private static BlockModelRenderer.EnumNeighborInfo[] makeEnumNeighborInfoValues() {
         BlockModelRenderer.EnumNeighborInfo[] values = new BlockModelRenderer.EnumNeighborInfo[6];
   ...
   public static enum Orientation {
      ...
      private Orientation(EnumFacing facingIn, boolean flip) {
         this.shape = facingIn.getIndex() + (flip ? EnumFacing._VALUES.length : 0);
      }
   }
   ...
   static enum VertexTranslations {
      ...
      private static final BlockModelRenderer.VertexTranslations[] VALUES = makeVertexTranslationsValues();

      private static BlockModelRenderer.VertexTranslations[] makeVertexTranslationsValues() {
         BlockModelRenderer.VertexTranslations[] values = new BlockModelRenderer.VertexTranslations[6];
         values[EnumFacing.DOWN.getIndex()] = DOWN;
         values[EnumFacing.UP.getIndex()] = UP;
         values[EnumFacing.NORTH.getIndex()] = NORTH;
         values[EnumFacing.SOUTH.getIndex()] = SOUTH;
