...
public class FogRenderer {
   ...
   private final BlockPos.MutableBlockPos fogBlockPos = new BlockPos.MutableBlockPos();
   ...
   private void updateSurfaceFog(Entity entityIn, World worldIn, float partialTicks) {
      ...
      if (this.mc.gameSettings.renderDistanceChunks >= 4) {
         ...
         float f5 = (float)(entityIn.getLook(partialTicks).x * d0);
         if (f5 < 0.0F) {
            f5 = 0.0F;
         }
   ...
   private void updateWaterFog(Entity entityIn, IWorldReaderBase worldIn, float partialTicks) {
      ...
      double d0 = entityIn.prevPosX + (entityIn.posX - entityIn.prevPosX) * (double)partialTicks + ActiveRenderInfo.getViewPositionX();
      double d1 = entityIn.prevPosY + (entityIn.posY - entityIn.prevPosY) * (double)partialTicks + ActiveRenderInfo.getViewPositionY();
      double d2 = entityIn.prevPosZ + (entityIn.posZ - entityIn.prevPosZ) * (double)partialTicks + ActiveRenderInfo.getViewPositionZ();
      int j = worldIn.getBiome(this.fogBlockPos.setPos(d0, d1, d2)).getWaterFogColor();
      ...
      if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPotionActive(MobEffects.BLINDNESS)) {
         ...
         if (entity instanceof EntityLivingBase) {
            if (entity instanceof EntityPlayerSP) {
               ...
               Biome biome = entityplayersp.world.getBiome(this.fogBlockPos.setPos(entityplayersp));
