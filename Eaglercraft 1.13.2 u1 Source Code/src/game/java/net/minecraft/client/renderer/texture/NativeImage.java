...
public final class NativeImage implements AutoCloseable {
   ...
   public NativeImage(NativeImage.PixelFormat pixelFormatIn, int widthIn, int heightIn, boolean initialize) {
      ...
      this.imageBuffer = MemoryUtil.memByteBuffer(this.imagePointer, this.size);
      this.imageIntBuffer = this.imageBuffer == null ? null : this.imageBuffer.asIntBuffer();
   }
   ...
   private NativeImage(NativeImage.PixelFormat pixelFormatIn, int widthIn, int heightIn, boolean stbiPointerIn, long pointer) {
      ...
      this.size = widthIn * heightIn * pixelFormatIn.getPixelSize();
      this.imageBuffer = MemoryUtil.memByteBuffer(pointer, this.size);
      this.imageIntBuffer = this.imageBuffer == null ? null : this.imageBuffer.asIntBuffer();
   }
   ...
   public int getPixelRGBA(int x, int y) {
      if (this.pixelFormat != NativeImage.PixelFormat.RGBA) {
         ...
         this.checkImage();
         return this.imageIntBuffer.get(x + y * this.width);
      } else {
   ...
   public void setPixelRGBA(int x, int y, int value) {
      if (this.pixelFormat != NativeImage.PixelFormat.RGBA) {
         ...
         this.checkImage();
         this.imageIntBuffer.put(x + y * this.width, value);
      } else {
         throw new IllegalArgumentException(String.format("(%s, %s) outside of image bounds (%s, %s)", x, y, this.width, this.height));
      }
   }
   ...
   public byte getPixelLuminanceOrAlpha(int x, int y) {
      if (!this.pixelFormat.hasLuminanceOrAlpha()) {
         ...
         return this.imageBuffer.get((x + y * this.width) * this.pixelFormat.getPixelSize() + this.pixelFormat.getOffsetAlphaBits() / 8);
      } else {
         throw new IllegalArgumentException(String.format("(%s, %s) outside of image bounds (%s, %s)", x, y, this.width, this.height));
      }
   }
   ...
   public void uploadTextureSub(int level, int xOffset, int yOffset, int unpackSkipPixels, int unpackSkipRows, int widthIn, int heightIn, boolean blur, boolean clamp, boolean mipmap) {
      ...
      GlStateManager.texSubImage2D(3553, level, xOffset, yOffset, widthIn, heightIn, this.pixelFormat.getGlFormat(), 5121, this.imageBuffer);
   }
   ...
   public void write(File fileIn) throws IOException {
      File file = fileIn.getParentFile();
      if (file != null) {
         file.mkdirs();
      }
      try (OutputStream outputstream = new FileOutputStream(fileIn)) {
         this.write(outputstream, fileIn.getAbsolutePath());
      }
   }
   ...
   public void write(OutputStream outputstream, String pathIn) throws IOException {
      if (!this.pixelFormat.isSerializable()) {
         ...
         try {
            NativeImage.WriteCallback nativeimage$writecallback = new NativeImage.WriteCallback(outputstream);

            try {
               if (!STBImageWrite.stbi_write_png_to_func(nativeimage$writecallback, 0L, this.getWidth(), this.getHeight(), this.pixelFormat.getPixelSize(), this.imageBuffer, 0)) {
                  throw new IOException("Could not write image to the PNG file \"" + pathIn + "\": " + STBImage.stbi_failure_reason());
               }
            } finally {
   ...
   static class WriteCallback extends STBIWriteCallback {
      ...
      private WriteCallback(OutputStream outputStreamIn) {
      ...
      public void invoke(long p_invoke_1_, long p_invoke_3_, int p_invoke_5_) {
         ...
         try {
            byte[] abyte = new byte[p_invoke_5_];
