...
public class RenderGuardian extends RenderLiving<EntityGuardian> {
      ...
      if (super.shouldRender(livingEntity, camera, camX, camY, camZ)) {
         ...
         if (livingEntity.hasTargetedEntity()) {
            ...
            if (entitylivingbase != null) {
               ...
               double d1 = (double)entitylivingbase.height * 0.5D + entitylivingbase.posY;
               double d2 = entitylivingbase.posZ;
               double d3 = livingEntity.posX;
               double d4 = (double)livingEntity.getEyeHeight() + livingEntity.posY;
               double d5 = livingEntity.posZ;
               if (camera.isBoundingBoxInFrustum(new AxisAlignedBB(d3, d4, d5, d0, d1, d2))) {
                  return true;
               }
            }
         }
   ...
   public void doRender(EntityGuardian entity, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      if (entitylivingbase != null) {
         ...
         double targetX = entitylivingbase.lastTickPosX + (entitylivingbase.posX - entitylivingbase.lastTickPosX) * (double)partialTicks;
         double targetY = (double)entitylivingbase.height * 0.5D + entitylivingbase.lastTickPosY + (entitylivingbase.posY - entitylivingbase.lastTickPosY) * (double)partialTicks;
         double targetZ = entitylivingbase.lastTickPosZ + (entitylivingbase.posZ - entitylivingbase.lastTickPosZ) * (double)partialTicks;
         double sourceX = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * (double)partialTicks;
         double sourceY = (double)f4 + entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * (double)partialTicks;
         double sourceZ = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * (double)partialTicks;
         double d30 = targetX - sourceX;
         double d31 = targetY - sourceY;
         double d32 = targetZ - sourceZ;
         double d0 = (double)MathHelper.sqrt(d30 * d30 + d31 * d31 + d32 * d32) + 1.0D;
         double d33 = d0 - 1.0D;
         if (d33 > 1.0E-4D) {
            d30 /= d33;
            d31 /= d33;
         ...
         float f5 = (float)Math.acos(d31);
         float f6 = (float)Math.atan2(d32, d30);
