   ...
   public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
      ...
      if (entityIn instanceof EntityLivingBase && ((EntityLivingBase)entityIn).isElytraFlying()) {
         ...
         if (entityIn.motionY < 0.0D) {
            double d0 = (double)MathHelper.sqrt(entityIn.motionX * entityIn.motionX + entityIn.motionY * entityIn.motionY + entityIn.motionZ * entityIn.motionZ);
            double d1 = d0 < 1.0E-4D ? 0.0D : entityIn.motionY / d0;
            f4 = 1.0F - (float)Math.pow(-d1, 1.5D);
         }
