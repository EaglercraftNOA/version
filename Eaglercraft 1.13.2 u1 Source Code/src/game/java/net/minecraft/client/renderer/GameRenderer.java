...
public class GameRenderer implements AutoCloseable, IResourceManagerReloadListener {
   ...
   private final Random random = new net.lax1dude.eaglercraft.v1_8.Random();
   ...
   private final Frustum renderFrustum = new Frustum();
   private final Matrix4f projectionMatrix = new Matrix4f();
   private final BlockPos.MutableBlockPos rainCenterPos = new BlockPos.MutableBlockPos();
   private final BlockPos.MutableBlockPos rainHeightPos = new BlockPos.MutableBlockPos();
   private final BlockPos.MutableBlockPos rainDownPos = new BlockPos.MutableBlockPos();
   ...
   private Matrix4f getProjectionMatrix(float partialTicks, boolean useFOVSetting, float farPlane) {
      return Matrix4f.perspective(this.getFOVModifier(partialTicks, useFOVSetting), (float)this.mc.mainWindow.getFramebufferWidth() / (float)this.mc.mainWindow.getFramebufferHeight(), 0.05F, farPlane, this.projectionMatrix);
   }
   ...
   private void orientCamera(float partialTicks) {
      ...
      if (entity instanceof EntityLivingBase && ((EntityLivingBase)entity).isPlayerSleeping()) {
         ...
         if (!this.mc.gameSettings.debugCamEnable) {
            BlockPos blockpos = this.cameraBlockPos.setPos(entity);
         ...
         if (this.mc.gameSettings.debugCamEnable) {
            ...
            for(int i = 0; i < 8; ++i) {
               ...
               if (raytraceresult != null) {
                  double d7 = raytraceresult.hitVec.x - d0;
                  double d8 = raytraceresult.hitVec.y - d1;
                  double d9 = raytraceresult.hitVec.z - d2;
   ...
   private void setupCameraTransform(float partialTicks) {
      ...
      GlStateManager.multMatrixf(this.getProjectionMatrix(partialTicks, true, this.farPlaneDistance * MathHelper.SQRT_2));
      ...
      if (f > 0.0F) {
         ...
         float f1 = 50.0F / (f * f + 50.0F) - f * 0.005F;
         f1 = f1 * f1;
   ...
   private void renderHand(float partialTicks) {
      if (!this.debugView) {
         GlStateManager.matrixMode(5889);
         GlStateManager.loadIdentity();
         GlStateManager.multMatrixf(this.getProjectionMatrix(partialTicks, false, this.farPlaneDistance * 2.0F));
         GlStateManager.matrixMode(5888);
         GlStateManager.loadIdentity();
   ...
   public void updateCameraAndRender(float partialTicks, long nanoTime, boolean renderWorldIn) {
      ...
      if (!this.mc.skipRenderWorld) {
         ...
         if (renderWorldIn && this.mc.world != null) {
            ...
            // audrey <3
            if (this.mc.isSingleplayer() && integratedserver != null && this.timeWorldIcon < Util.milliTime() - 1000L) {
               this.timeWorldIcon = Util.milliTime();
               if (!integratedserver.isWorldIconSet()) {
                  this.createWorldIcon(integratedserver);
               }
            }
   ...
   // audrey <3
   private void createWorldIcon(IntegratedServer integratedserver) {
      if (this.mc.worldRenderer.getRenderedChunks() > 10 && this.mc.worldRenderer.hasNoChunkUpdates() && !integratedserver.isWorldIconSet()) {
         ...
         SimpleResource.RESOURCE_IO_EXECUTOR.execute(() -> {
            ...
            try (NativeImage nativeimage1 = new NativeImage(64, 64, false)) {
               ...
               nativeimage1.write(integratedserver.getWorldIconFile());
            } catch (IOException ioexception) {
   ...
   private void updateCameraAndRender(float partialTicks, long nanoTime) {
      ...
      ActiveRenderInfo.updateRenderInfo(this.mc.player, this.mc.gameSettings.thirdPersonView == 2, this.farPlaneDistance);
      this.mc.profiler.endStartSection("frustum");
      Entity viewEnt = this.mc.getRenderViewEntity();
      float lvx = (float)(viewEnt != null ? viewEnt.posX : 0.0D);
      float lvy = (float)(viewEnt != null ? viewEnt.posY : 0.0D);
      float lvz = (float)(viewEnt != null ? viewEnt.posZ : 0.0D);
      ClippingHelperImpl.setReferenceFrame(lvx, lvy, lvz,
            viewEnt != null ? viewEnt.rotationPitch : 0.0F,
            viewEnt != null ? viewEnt.rotationYaw : 0.0F,
            (float)this.mc.gameSettings.fovSetting, this.farPlaneDistance);
      ...
      if (this.mc.gameSettings.renderDistanceChunks >= 4) {
         ...
         GlStateManager.matrixMode(5889);
         GlStateManager.loadIdentity();
         GlStateManager.multMatrixf(this.getProjectionMatrix(partialTicks, true, this.farPlaneDistance * 2.0F));
         GlStateManager.matrixMode(5888);
   ...
   private void renderCloudsCheck(WorldRenderer renderGlobalIn, float partialTicks, double viewEntityX, double viewEntityY, double viewEntityZ) {
      if (this.mc.gameSettings.shouldRenderClouds() != 0) {
         ...
         GlStateManager.matrixMode(5889);
         GlStateManager.loadIdentity();
         GlStateManager.multMatrixf(this.getProjectionMatrix(partialTicks, true, this.farPlaneDistance * 4.0F));
         GlStateManager.matrixMode(5888);
         GlStateManager.pushMatrix();
         this.fogRenderer.setupFog(0, partialTicks);
   ...
   private void addRainParticles() {
      ...
      if (f != 0.0F) {
         ...
         BlockPos.MutableBlockPos blockpos = this.rainCenterPos.setPos(entity);
         int i = 10;
         double d0 = 0.0D;
         double d1 = 0.0D;
         double d2 = 0.0D;
         ...
         for(int l = 0; l < k; ++l) {
            int i1 = blockpos.getX() + this.random.nextInt(10) - this.random.nextInt(10);
            int j1 = blockpos.getZ() + this.random.nextInt(10) - this.random.nextInt(10);
            int k1 = iworldreaderbase.getHeight(Heightmap.Type.MOTION_BLOCKING, i1, j1);
            BlockPos.MutableBlockPos blockpos1 = this.rainHeightPos.setPos(i1, k1, j1);
            Biome biome = iworldreaderbase.getBiome(blockpos1);
            BlockPos.MutableBlockPos blockpos2 = this.rainDownPos.setPos(i1, k1 - 1, j1);
            if (blockpos1.getY() <= blockpos.getY() + 10 && blockpos1.getY() >= blockpos.getY() - 10 && biome.getPrecipitation() == Biome.RainType.RAIN && biome.getTemperature(blockpos1) >= 0.15F) {
               ...
               if (d5 > -Double.MAX_VALUE) {
                  if (!ifluidstate.isTagged(FluidTags.LAVA) && iblockstate.getBlock() != Blocks.MAGMA_BLOCK) {
                     ...
                     this.mc.world.addParticle(Particles.RAIN, (double)blockpos2.getX() + d3, (double)((float)blockpos2.getY() + 0.1F) + d5, (double)blockpos2.getZ() + d4, 0.0D, 0.0D, 0.0D);
                  } else {
                     this.mc.world.addParticle(Particles.SMOKE, (double)blockpos1.getX() + d3, (double)((float)blockpos1.getY() + 0.1F) - d6, (double)blockpos1.getZ() + d4, 0.0D, 0.0D, 0.0D);
                  }
               }
            }
         }
         ...
         if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
            ...
            if (d1 > (double)(blockpos.getY() + 1) && iworldreaderbase.getHeight(Heightmap.Type.MOTION_BLOCKING, blockpos.getX(), blockpos.getZ()) > MathHelper.floor((float)blockpos.getY())) {
