...
public class BlockFluidRenderer {
   private final BlockFluidRenderer.RenderContext renderContext = new RenderContext();
   ...
   private static boolean isAdjacentFluidSameAs(IBlockReader worldIn, BlockPos pos, IFluidState state) {
      IFluidState ifluidstate = worldIn.getFluidState(pos);
      return ifluidstate.getFluid().isEquivalentTo(state.getFluid());
   }

   private static boolean func_209556_a(IBlockReader reader, BlockPos blockpos, EnumFacing face, float heightIn) {
   ...
   public boolean render(IWorldReader worldIn, BlockPos pos, BufferBuilder buffer, IFluidState state) {
      BlockFluidRenderer.RenderContext blockfluidrenderer$rendercontext = this.renderContext;
      ...
      boolean flag1 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.UP), state);
      boolean flag2 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.DOWN), state) && !func_209556_a(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.DOWN), EnumFacing.DOWN, 0.8888889F);
      boolean flag3 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.NORTH), state);
      boolean flag4 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.SOUTH), state);
      boolean flag5 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.WEST), state);
      boolean flag6 = !isAdjacentFluidSameAs(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.EAST), state);
      if (!flag1 && !flag2 && !flag6 && !flag5 && !flag3 && !flag4) {
         ...
         float f5 = 0.8F;
         float f6 = 0.6F;
         float f7 = this.getFluidHeight(worldIn, pos, state.getFluid(), blockfluidrenderer$rendercontext.heightPos, blockfluidrenderer$rendercontext.heightUpPos);
         float f8 = this.getFluidHeight(worldIn, blockfluidrenderer$rendercontext.inputPos.setPos(pos).move(EnumFacing.SOUTH), state.getFluid(), blockfluidrenderer$rendercontext.heightPos, blockfluidrenderer$rendercontext.heightUpPos);
         float f9 = this.getFluidHeight(worldIn, blockfluidrenderer$rendercontext.inputPos.setPos(pos).move(EnumFacing.EAST).move(EnumFacing.SOUTH), state.getFluid(), blockfluidrenderer$rendercontext.heightPos, blockfluidrenderer$rendercontext.heightUpPos);
         float f10 = this.getFluidHeight(worldIn, blockfluidrenderer$rendercontext.inputPos.setPos(pos).move(EnumFacing.EAST), state.getFluid(), blockfluidrenderer$rendercontext.heightPos, blockfluidrenderer$rendercontext.heightUpPos);
         double d0 = (double)pos.getX();
         double d1 = (double)pos.getY();
         double d2 = (double)pos.getZ();
         float f11 = 0.001F;
         if (flag1 && !func_209556_a(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.UP), EnumFacing.UP, Math.min(Math.min(f7, f8), Math.min(f9, f10)))) {
            flag7 = true;
            f7 -= 0.001F;
            f8 -= 0.001F;
            f9 -= 0.001F;
            ...
            int i2 = this.getCombinedLightUpMax(worldIn, pos, blockfluidrenderer$rendercontext.lightPos);
            int j2 = i2 >> 16 & '\uffff';
            int k2 = i2 & '\uffff';
            float f42 = 1.0F * f;
            float f43 = 1.0F * f1;
            ...
            if (state.shouldRenderSides(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.UP))) {
               buffer.pos(d0 + 0.0D, d1 + (double)f7, d2 + 0.0D).color(f42, f43, f24, 1.0F).tex((double)f12, (double)f16).lightmap(j2, k2).endVertex();
               buffer.pos(d0 + 1.0D, d1 + (double)f10, d2 + 0.0D).color(f42, f43, f24, 1.0F).tex((double)f15, (double)f19).lightmap(j2, k2).endVertex();
               buffer.pos(d0 + 1.0D, d1 + (double)f9, d2 + 1.0D).color(f42, f43, f24, 1.0F).tex((double)f14, (double)f18).lightmap(j2, k2).endVertex();
               buffer.pos(d0 + 0.0D, d1 + (double)f8, d2 + 1.0D).color(f42, f43, f24, 1.0F).tex((double)f13, (double)f17).lightmap(j2, k2).endVertex();
         ...
         if (flag2) {
            ...
            int j1 = this.getCombinedLightUpMax(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(EnumFacing.DOWN), blockfluidrenderer$rendercontext.lightPos);
            int k1 = j1 >> 16 & '\uffff';
            int l1 = j1 & '\uffff';
            float f39 = 0.5F * f;
            float f40 = 0.5F * f1;
         ...
         for(int i1 = 0; i1 < 4; ++i1) {
            ...
            if (flag8 && !func_209556_a(worldIn, blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(enumfacing), enumfacing, Math.max(f35, f37))) {
               flag7 = true;
               BlockPos blockpos = blockfluidrenderer$rendercontext.sidePos.setPos(pos).move(enumfacing);
               ...
               int j = this.getCombinedLightUpMax(worldIn, blockpos, blockfluidrenderer$rendercontext.lightPos);
               int k = j >> 16 & '\uffff';
               int l = j & '\uffff';
   ...
   private int getCombinedLightUpMax(IWorldReader reader, BlockPos pos, BlockPos.MutableBlockPos upPos) {
      int i = reader.getCombinedLight(pos, 0);
      int j = reader.getCombinedLight(upPos.setPos(pos).move(EnumFacing.UP), 0);
      int k = i & 255;
      int l = j & 255;
      int i1 = i >> 16 & 255;
      int j1 = j >> 16 & 255;
   ...
   private float getFluidHeight(IWorldReaderBase reader, BlockPos pos, Fluid fluidIn, BlockPos.MutableBlockPos blockpos, BlockPos.MutableBlockPos blockpos1) {
      int i = 0;
      float f = 0.0F;

      for(int j = 0; j < 4; ++j) {
         blockpos.setPos(pos).move(-(j & 1), 0, -(j >> 1 & 1));
         if (reader.getFluidState(blockpos1.setPos(blockpos).move(EnumFacing.UP)).getFluid().isEquivalentTo(fluidIn)) {
            return 1.0F;
         }
   ...
   static class RenderContext {
      final BlockPos.MutableBlockPos sidePos = new BlockPos.MutableBlockPos();
      final BlockPos.MutableBlockPos inputPos = new BlockPos.MutableBlockPos();
      final BlockPos.MutableBlockPos heightPos = new BlockPos.MutableBlockPos();
      final BlockPos.MutableBlockPos heightUpPos = new BlockPos.MutableBlockPos();
