...
public abstract class Render<T extends Entity> {
   ...
   private final BlockPos.MutableBlockPos shadowBlockPosDown = new BlockPos.MutableBlockPos();
      ...
      for(int k1 = i; k1 <= j; ++k1) {
         for(int l1 = k; l1 <= l; ++l1) {
            for(int i2 = i1; i2 <= j1; ++i2) {
               BlockPos.MutableBlockPos blockpos = this.shadowBlockPos.setPos(k1, l1, i2);
               BlockPos.MutableBlockPos blockpos1 = this.shadowBlockPosDown.setPos(k1, l1 - 1, i2);
               ...
               if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE && iworldreaderbase.getLight(blockpos) > 3) {
                  this.renderShadowSingle(iblockstate, x, y, z, blockpos, blockpos1, shadowAlpha, f, d2, d3, d4);
               }
            }
         }
      }
   ...
   private void renderShadowSingle(IBlockState state, double p_188299_2_, double p_188299_4_, double p_188299_6_, BlockPos p_188299_8_, BlockPos p_188299_9_, float p_188299_10_, float p_188299_11_, double p_188299_12_, double p_188299_14_, double p_188299_16_) {
      if (state.isFullCube()) {
         VoxelShape voxelshape = state.getShape(this.getWorldFromRenderManager(), p_188299_9_);
         if (!voxelshape.isEmpty()) {
            Tessellator tessellator = Tessellator.getInstance();
            BufferBuilder bufferbuilder = tessellator.getBuffer();
            double d0 = ((double)p_188299_10_ - (p_188299_4_ - ((double)p_188299_8_.getY() + p_188299_14_)) / 2.0D) * 0.5D * (double)this.getWorldFromRenderManager().getBrightness(p_188299_8_);
            if (!(d0 < 0.0D)) {
               if (d0 > 1.0D) {
                  d0 = 1.0D;
               }
               ...
               double d1 = (double)p_188299_8_.getX() + axisalignedbb.minX + p_188299_12_;
               double d2 = (double)p_188299_8_.getX() + axisalignedbb.maxX + p_188299_12_;
               double d3 = (double)p_188299_8_.getY() + axisalignedbb.minY + p_188299_14_ + 0.015625D;
               double d4 = (double)p_188299_8_.getZ() + axisalignedbb.minZ + p_188299_16_;
               double d5 = (double)p_188299_8_.getZ() + axisalignedbb.maxZ + p_188299_16_;
               float f = (float)((p_188299_2_ - d1) / 2.0D / (double)p_188299_11_ + 0.5D);
               float f1 = (float)((p_188299_2_ - d2) / 2.0D / (double)p_188299_11_ + 0.5D);
               float f2 = (float)((p_188299_6_ - d4) / 2.0D / (double)p_188299_11_ + 0.5D);
               float f3 = (float)((p_188299_6_ - d5) / 2.0D / (double)p_188299_11_ + 0.5D);
