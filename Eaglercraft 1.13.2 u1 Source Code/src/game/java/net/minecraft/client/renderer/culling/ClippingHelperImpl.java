...
public class ClippingHelperImpl extends ClippingHelper {
   ...
   private float refCamX;
   private float refCamY;
   private float refCamZ;
   private float refCamPitch;
   private float refCamYaw;
   private float refCamFovScale;
   private float refFarPlane;
   ...
   public static void markDirty() {
      field_195630_e.dirty = true;
   }

   public static void setReferenceFrame(float camX, float camY, float camZ, float pitch, float yaw, float fovScale, float farPlane) {
      ClippingHelperImpl h = field_195630_e;
      if (camX != h.refCamX || camY != h.refCamY || camZ != h.refCamZ
            || pitch != h.refCamPitch || yaw != h.refCamYaw
            || fovScale != h.refCamFovScale || farPlane != h.refFarPlane) {
         h.refCamX = camX;
         h.refCamY = camY;
         h.refCamZ = camZ;
         h.refCamPitch = pitch;
         h.refCamYaw = yaw;
         h.refCamFovScale = fovScale;
         h.refFarPlane = farPlane;
         h.dirty = true;
      }
   }
   ...
   public void init() {
      ...
      net.lax1dude.eaglercraft.v1_8.opengl.GlStateManager.getFloat(2983, afloat);
      net.lax1dude.eaglercraft.v1_8.opengl.GlStateManager.getFloat(2982, afloat1);
