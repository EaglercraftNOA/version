...
import com.arlenh7.eaglercraft.v1_17.java.util.concurrent.TimeoutException;
...
import net.minecraft.resources.EaglerWorldDataPackFinder;
...
public abstract class MinecraftServer implements IThreadListener, ISnooperInfo, ICommandSource, Runnable {
   ...
   private static final GameProfileRepository offlineProfileRepository = new GameProfileRepository() {
      public void findProfilesByNames(String[] names, Agent agent, ProfileLookupCallback callback) {
         for(String name : names) {
            callback.onProfileLookupFailed(new GameProfile((UUID)null, name), new IllegalStateException("Offline profile lookup"));
         }
      }
   };
   private static final YggdrasilAuthenticationService offlineAuthService = null;
   private static final MinecraftSessionService offlineSessionService = new MinecraftSessionService() {
   ...
   private EaglerWorldDataPackFinder datapackFinder;
   ...
   public MinecraftServer(String worldName) {
      this(null, null, DataFixesManager.getDataFixer(), new Commands(false), offlineAuthService, offlineSessionService, offlineProfileRepository, new PlayerProfileCache(offlineProfileRepository, new File(WorldsDB.newVFile(USER_CACHE_FILE.getName()).getPath())));
      this.setFolderName(worldName);
      this.setWorldName(worldName);
   }

   public MinecraftServer(@Nullable VFile2 anvilFileIn, Proxy serverProxyIn, DataFixer dataFixerIn, Commands commandManagerIn, YggdrasilAuthenticationService authServiceIn, MinecraftSessionService sessionServiceIn, GameProfileRepository profileRepoIn, PlayerProfileCache profileCacheIn) {
      ...
      this.anvilConverterForAnvilFile = new AnvilSaveConverter(WorldsDB.newVFile("worlds"), WorldsDB.newVFile("backups"), dataFixerIn);
   ...
   public void func_212369_a(ISaveHandler p_212369_1_, WorldSavedDataStorage p_212369_2_, WorldInfo p_212369_3_, WorldSettings p_212369_4_) {
      if (this.isDemo()) {
         this.worlds.put(DimensionType.OVERWORLD, (new WorldServerDemo(this, p_212369_1_, p_212369_2_, p_212369_3_, DimensionType.OVERWORLD, this.profiler)).init());
      } else {
         this.worlds.put(DimensionType.OVERWORLD, (new WorldServer(this, p_212369_1_, p_212369_2_, p_212369_3_, DimensionType.OVERWORLD, this.profiler)).init());
      }
      ...
      WorldServerMulti worldservermulti = (new WorldServerMulti(this, p_212369_1_, DimensionType.NETHER, worldserver, this.profiler)).init();
      ...
      WorldServerMulti worldservermulti1 = (new WorldServerMulti(this, p_212369_1_, DimensionType.THE_END, worldserver, this.profiler)).init();
   ...
   public void loadDataPacks(VFile2 p_195560_1_, WorldInfo p_195560_2_) {
      this.resourcePacks.addPackFinder(new ServerPackFinder());
      this.datapackFinder = new EaglerWorldDataPackFinder(p_195560_1_);
   ...
   public void initialWorldChunkLoad(WorldSavedDataStorage p_71222_1_) {
      ...
      WorldServer worldserver = this.getWorld(DimensionType.OVERWORLD);
      LOGGER.info("Preparing start region for dimension " + DimensionType.getKey(worldserver.dimension.getType()));
      BlockPos blockpos = worldserver.getSpawnPoint();
      List<ChunkPos> list = Lists.newArrayList();
      Set<ChunkPos> set = Sets.newConcurrentHashSet();
      Stopwatch stopwatch = Stopwatch.createStarted();

      for(int j1 = -32; j1 <= 32 && this.isServerRunning(); j1 += 16) {
         for(int k1 = -32; k1 <= 32 && this.isServerRunning(); k1 += 16) {
         ...
         while(!completablefuture.isDone()) {
            try {
               ...
               this.setCurrentTaskAndPercentDone(new TextComponentTranslation("menu.preparingSpawn"), set.size() * 100 / 25);
            }
         }
      ...
      for(DimensionType dimensiontype : DimensionType.getAll()) {
         ForcedChunksSaveData forcedchunkssavedata = p_71222_1_.get(dimensiontype, ForcedChunksSaveData::new, "chunks");
         if (forcedchunkssavedata != null) {
            WorldServer worldserver1 = this.getWorld(dimensiontype);
            LongIterator longiterator = forcedchunkssavedata.getChunks().iterator();

            while(longiterator.hasNext()) {
               this.setCurrentTaskAndPercentDone(new TextComponentTranslation("menu.loadingForcedChunks", dimensiontype), forcedchunkssavedata.getChunks().size() * 100 / 625);
               long l1 = longiterator.nextLong();
               ChunkPos chunkpos = new ChunkPos(l1);
               worldserver1.getChunkProvider().getChunk(chunkpos.x, chunkpos.z, true, true);
            }
         }
      }
   ...
   public void setResourcePackFromWorld(String worldNameIn, ISaveHandler saveHandlerIn) {
      File file1 = new File(saveHandlerIn.getWorldDirectory().getPath(), "resources.zip");
      if (file1.isFile()) {
         try {
   ...
   public void saveAllWorlds(boolean isSilent) {
      for(WorldServer worldserver : this.getWorlds()) {
         if (worldserver != null) {
            if (!isSilent) {
               LOGGER.info("Saving chunks for level '{}'/{}", worldserver.getWorldInfo().getWorldName(), DimensionType.getKey(worldserver.dimension.getType()));
            }
   ...
   public void applyServerIconToResponse(ServerStatusResponse response) {
      ...
      if (!file1.exists()) {
         file1 = new File(this.getActiveAnvilConverter().getFile(this.getFolderName(), "icon.png").getPath());
      }

      if (file1.isFile()) {
         try {
            byte[] abyte = FileUtils.readFileToByteArray(file1);
            ImageData imagedata = ImageData.loadImageFile(abyte, "image/png");
            Validate.validState(imagedata.width == 64, "Must be 64 pixels wide");
            Validate.validState(imagedata.height == 64, "Must be 64 pixels high");
            response.setFavicon("data:image/png;base64," + Base64.encodeBase64String(abyte));
   ...
   public File getWorldIconFile() {
      return new File(this.getActiveAnvilConverter().getFile(this.getFolderName(), "icon.png").getPath());
   }
   ...
   public void tick(BooleanSupplier hasTimeLeft) {
      ...
      if (i - this.nanoTimeSinceStatusRefresh >= 5000000000L) {
         ...
         for(int l = agameprofile.length - 1; l > 0; --l) {
            int i1 = this.random.nextInt(l + 1);
            GameProfile gameprofile = agameprofile[l];
            agameprofile[l] = agameprofile[i1];
            agameprofile[i1] = gameprofile;
         }
   ...
   public static void main(String[] p_main_0_) {
      ...
      try {
         ...
         PlayerProfileCache playerprofilecache = new PlayerProfileCache(offlineProfileRepository, new File(s1, USER_CACHE_FILE.getName()));
         final DedicatedServer dedicatedserver = new DedicatedServer(new File(s1), DataFixesManager.getDataFixer(), offlineAuthService, offlineSessionService, offlineProfileRepository, playerprofilecache);
         if (s != null) {
   ...
   public void fillSnooper(Snooper snooper) {
      ...
      if (this.playerList != null) {
         ...
         snooper.addClientStat("players_seen", this.playerList.getAvailablePlayerDat().length);
      }
      ...
      for(WorldServer worldserver : this.getWorlds()) {
         if (worldserver != null) {
            ...
            snooper.addClientStat("world[" + i + "][dimension]", worldserver.dimension.getType());
            snooper.addClientStat("world[" + i + "][mode]", worldinfo.getGameType());
            snooper.addClientStat("world[" + i + "][difficulty]", worldserver.getDifficulty());
            ...
            snooper.addClientStat("world[" + i + "][chunks_loaded]", worldserver.getChunkProvider().getLoadedChunkCount());
            ++i;
         }
      }
