...
public class EntityTippedArrow extends EntityArrow {
   ...
   private void spawnPotionParticles(int particleCount) {
      ...
      if (i != -1 && particleCount > 0) {
         ...
         for(int j = 0; j < particleCount; ++j) {
            this.world.addParticle(Particles.ENTITY_EFFECT, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, d0, d1, d2);
         }
   ...
   public void writeAdditional(NBTTagCompound compound) {
      ...
      if (this.potion != PotionTypes.EMPTY && this.potion != null) {
         compound.putString("Potion", IRegistry.POTION.getKey(this.potion).toString());
      }
