...
public abstract class EntityPlayer extends EntityLivingBase {
   ...
   public float getDigSpeed(IBlockState state) {
      ...
      if (PotionUtil.hasMiningSpeedup(this)) {
         f *= 1.0F + (float)(PotionUtil.getMiningSpeedup(this) + 1) * 0.2F;
      }
   ...
   public void writeAdditional(NBTTagCompound compound) {
      ...
      compound.putInt("DataVersion", 1631);
   ...
   protected void damageEntity(DamageSource damageSrc, float damageAmount) {
      if (!this.isInvulnerableTo(damageSrc)) {
         ...
         if (f1 > 0.0F && f1 < 3.4028235E37F) {
            this.addStat(StatList.DAMAGE_ABSORBED, Math.round(f1 * 10.0F));
         }
   ...
   public EntityPlayer.SleepResult trySleep(BlockPos bedLocation) {
      EnumFacing enumfacing = this.world.getBlockState(bedLocation).get(BlockHorizontal.HORIZONTAL_FACING);
      if (!this.world.isRemote) {
   ...
   public static UUID getOfflineUUID(String username) {
      return EaglerUUIDHelper.nameUUIDFromBytes(("OfflinePlayer:" + username).getBytes(StandardCharsets.UTF_8));
   }
