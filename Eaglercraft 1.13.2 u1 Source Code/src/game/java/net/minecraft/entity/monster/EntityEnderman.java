...
public class EntityEnderman extends EntityMob {
   ...
   public void writeAdditional(NBTTagCompound compound) {
      ...
      if (iblockstate != null) {
         compound.put("carriedBlockState", NBTUtil.writeBlockState(iblockstate));
      }
   ...
   public void livingTick() {
      if (this.world.isRemote) {
         for(int i = 0; i < 2; ++i) {
            this.world.addParticle(Particles.PORTAL, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height - 0.25D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, (this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(), (this.rand.nextDouble() - 0.5D) * 2.0D);
         }
      }
