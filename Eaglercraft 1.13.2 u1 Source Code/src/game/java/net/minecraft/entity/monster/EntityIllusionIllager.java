   ...
   public void livingTick() {
      ...
      if (this.world.isRemote && this.isInvisible()) {
         ...
         if (this.hurtTime != 1 && this.ticksExisted % 1200 != 0) {
            ...
            for(int l = 0; l < 16; ++l) {
               this.world.addParticle(Particles.CLOUD, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, 0.0D, 0.0D, 0.0D);
            }
