...
public class EntityGuardian extends EntityMob {
   ...
   public void livingTick() {
      if (this.world.isRemote) {
         ...
         if (this.isMoving() && this.isInWater()) {
            ...
            for(int i = 0; i < 2; ++i) {
               this.world.addParticle(Particles.BUBBLE, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3d.x * 1.5D, this.posY + this.rand.nextDouble() * (double)this.height - vec3d.y * 1.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width - vec3d.z * 1.5D, 0.0D, 0.0D, 0.0D);
            }
         }
         ...
         if (this.hasTargetedEntity()) {
            ...
            if (entitylivingbase != null) {
               ...
               while(d4 < d3) {
                  ...
                  this.world.addParticle(Particles.BUBBLE, this.posX + d0 * d4, this.posY + d1 * d4 + (double)this.getEyeHeight(), this.posZ + d2 * d4, 0.0D, 0.0D, 0.0D);
               }
            }
         }
      }
   ...
   public boolean canSpawn(IWorld worldIn, boolean fromSpawner) {
      return (this.rand.nextInt(20) == 0 || !worldIn.canBlockSeeSky(new BlockPos(this))) && super.canSpawn(worldIn, fromSpawner);
   }
