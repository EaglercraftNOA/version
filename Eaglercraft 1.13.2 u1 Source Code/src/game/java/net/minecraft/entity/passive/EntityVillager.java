...
public class EntityVillager extends EntityAgeable implements INpc, IMerchant {
   ...
   public boolean processInteract(EntityPlayer player, EnumHand hand) {
      ...
      if (flag) {
         ...
         return true;
      } else if (itemstack.getItem() != Items.VILLAGER_SPAWN_EGG && this.isAlive() && !this.isTrading() && !this.isChild()) {
   ...
   public void setRevengeTarget(@Nullable EntityLivingBase livingBase) {
      ...
      if (this.village != null && livingBase != null) {
         ...
         if (livingBase instanceof EntityPlayer) {
            ...
            this.village.modifyPlayerReputation(((EntityPlayer)livingBase).getGameProfile().getName(), i);
            if (this.isAlive()) {
   ...
   public void handleStatusUpdate(byte id) {
      if (id == 12) {
         this.addParticle(Particles.HEART);
      } else if (id == 13) {
         this.addParticle(Particles.ANGRY_VILLAGER);
      } else if (id == 14) {
         this.addParticle(Particles.HAPPY_VILLAGER);
      } else {
   ...
   @OnlyIn(Dist.CLIENT)
   private void addParticle(IParticleData p_195400_1_) {
      for(int i = 0; i < 5; ++i) {
         double d0 = this.rand.nextGaussian() * 0.02D;
         double d1 = this.rand.nextGaussian() * 0.02D;
         double d2 = this.rand.nextGaussian() * 0.02D;
         this.world.addParticle(p_195400_1_, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + 1.0D + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d0, d1, d2);
      }
