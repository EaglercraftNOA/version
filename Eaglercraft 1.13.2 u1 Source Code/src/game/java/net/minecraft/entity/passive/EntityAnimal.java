...
public abstract class EntityAnimal extends EntityAgeable implements IAnimal {
   ...
   public void livingTick() {
      ...
      if (this.inLove > 0) {
         ...
         if (this.inLove % 10 == 0) {
            ...
            this.world.addParticle(Particles.HEART, this.posX + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, this.posY + 0.5D + (double)(this.rand.nextFloat() * this.height), this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d0, d1, d2);
         }
      }
   ...
   public boolean canSpawn(IWorld worldIn, boolean fromSpawner) {
      ...
      return worldIn.getBlockState(blockpos.down()).getBlock() == this.spawnableBlock && worldIn.getLightSubtracted(blockpos, 0) > 8 && super.canSpawn(worldIn, fromSpawner);
   }
