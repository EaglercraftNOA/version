...
public class EntityParrot extends EntityShoulderRiding implements IFlyingAnimal {
      ...
      if (!parrotIn.isSilent() && worldIn.rand.nextInt(50) == 0) {
         ...
         if (!list.isEmpty()) {
            ...
            if (!entityliving.isSilent()) {
               SoundEvent soundevent = getMimicSound(entityliving.getType());
   ...
   public boolean canSpawn(IWorld worldIn, boolean fromSpawner) {
      ...
      return block instanceof BlockLeaves || block == Blocks.GRASS || block instanceof BlockLog || block == Blocks.AIR && super.canSpawn(worldIn, fromSpawner);
   }
