...
public class EntityRabbit extends EntityAnimal {
   ...
   public void setRabbitType(int rabbitTypeId) {
      if (rabbitTypeId == 99) {
         ...
         if (!this.hasCustomName()) {
            this.setCustomName(new TextComponentTranslation(Util.makeTranslationKey("entity", KILLER_BUNNY)));
         }
      }
   ...
   public boolean canSpawn(IWorld worldIn, boolean fromSpawner) {
      ...
      return block != Blocks.GRASS && block != Blocks.SNOW && block != Blocks.SAND ? super.canSpawn(worldIn, fromSpawner) : true;
   }
