...
public class EntityType<T extends Entity> {
   ...
   public static <T extends Entity> EntityType<T> register(String id, EntityType.Builder<T> builder) {
      ...
      IRegistry.ENTITY_TYPE.put(new ResourceLocation(id), entitytype);
      return entitytype;
   }

   @Nullable
   public static ResourceLocation getId(EntityType<?> entityTypeIn) {
      return IRegistry.ENTITY_TYPE.getKey(entityTypeIn);
   }

   @Nullable
   public static EntityType<?> getById(String id) {
      return IRegistry.ENTITY_TYPE.get(ResourceLocation.tryCreate(id));
   }
   ...
   @Nullable
   public T spawn(World worldIn, @Nullable NBTTagCompound nbt, @Nullable ITextComponent customName, @Nullable EntityPlayer p_208050_4_, BlockPos p_208050_5_, boolean p_208050_6_, boolean p_208050_7_) {
      T t = this.create(worldIn, nbt, customName, p_208050_4_, p_208050_5_, p_208050_6_, p_208050_7_);
      worldIn.spawnEntity(t);
      return t;
   }

   @Nullable
   public T create(World worldIn, @Nullable NBTTagCompound nbt, @Nullable ITextComponent customName, @Nullable EntityPlayer p_210761_4_, BlockPos pos, boolean p_210761_6_, boolean p_210761_7_) {
      ...
      if (t == null) {
         ...
         if (customName != null && t instanceof EntityLivingBase) {
            t.setCustomName(customName);
         }

         applyItemNBT(worldIn, p_210761_4_, t, nbt);
         return t;
      }
   }
   ...
   protected static double func_208051_a(IWorldReaderBase p_208051_0_, BlockPos p_208051_1_, boolean p_208051_2_, AxisAlignedBB p_208051_3_) {
      ...
      Stream<VoxelShape> stream = p_208051_0_.getCollisionBoxes((Entity)null, axisalignedbb);
      return 1.0D + VoxelShapes.func_212437_a(EnumFacing.Axis.Y, p_208051_3_, stream, p_208051_2_ ? -2.0D : -1.0D);
   }

   public static void applyItemNBT(World worldIn, @Nullable EntityPlayer player, @Nullable Entity spawnedEntity, @Nullable NBTTagCompound itemNBT) {
      if (itemNBT != null && itemNBT.contains("EntityTag", 10)) {
         ...
         if (minecraftserver != null && spawnedEntity != null) {
            if (worldIn.isRemote || !spawnedEntity.ignoreItemEntityData() || player != null && minecraftserver.getPlayerList().canSendCommands(player.getGameProfile())) {
               NBTTagCompound nbttagcompound = spawnedEntity.writeWithoutTypeId(new NBTTagCompound());
               UUID uuid = spawnedEntity.getUniqueID();
               nbttagcompound.merge(itemNBT.getCompound("EntityTag"));
               spawnedEntity.setUniqueId(uuid);
               spawnedEntity.read(nbttagcompound);
            }
         }
      }
   }
   ...
   public ITextComponent getName() {
      if (this.name == null) {
         this.name = new TextComponentTranslation(this.getTranslationKey());
      }
   ...
   public static Entity create(World worldIn, ResourceLocation id) {
      return create(worldIn, IRegistry.ENTITY_TYPE.get(id));
   }
