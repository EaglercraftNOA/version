...
public final class Ingredient implements Predicate<ItemStack> {
   ...
   public static Ingredient read(PacketBuffer buffer) {
      ...
      Ingredient.IItemList[] aingredient$iitemlist = new Ingredient.IItemList[i];

      for(int j = 0; j < i; ++j) {
         aingredient$iitemlist[j] = new Ingredient.SingleItemList(buffer.readItemStack());
      }

      return fromItemListStream(Arrays.stream(aingredient$iitemlist));
   }
   ...
   public static Ingredient.IItemList deserializeItemList(JsonObject json) {
      if (json.has("item") && json.has("tag")) {
         ...
         Item item = IRegistry.ITEM.get(resourcelocation1);
         if (item == null) {
   ...
   static class SingleItemList implements Ingredient.IItemList {
      ...
      public JsonObject serialize() {
         ...
         jsonobject.addProperty("item", IRegistry.ITEM.getKey(this.stack.getItem()).toString());
         return jsonobject;
      }
   }
