...
public final class VoxelShapes {
   ...
   public static boolean compare(VoxelShape shape1, VoxelShape shape2, IBooleanFunction function) {
      ...
      } else if (function == IBooleanFunction.AND) {
   ...
   private static boolean intersects(VoxelShape shape1, VoxelShape shape2) {
      int i = shape1.part.getSize(EnumFacing.Axis.X);
      int j = shape1.part.getSize(EnumFacing.Axis.Y);
      int k = shape1.part.getSize(EnumFacing.Axis.Z);
      int l = shape2.part.getSize(EnumFacing.Axis.X);
      int i1 = shape2.part.getSize(EnumFacing.Axis.Y);
      int j1 = shape2.part.getSize(EnumFacing.Axis.Z);
      for(int k1 = 0; k1 < i; ++k1) {
         double d0 = shape1.getValueUnchecked(EnumFacing.Axis.X, k1);
         double d1 = shape1.getValueUnchecked(EnumFacing.Axis.X, k1 + 1);
         for(int l1 = 0; l1 < l; ++l1) {
            double d2 = shape2.getValueUnchecked(EnumFacing.Axis.X, l1);
            double d3 = shape2.getValueUnchecked(EnumFacing.Axis.X, l1 + 1);
            if (d1 <= d2 + 1.0E-7D || d3 <= d0 + 1.0E-7D) {
               continue;
            }
            for(int i2 = 0; i2 < j; ++i2) {
               double d4 = shape1.getValueUnchecked(EnumFacing.Axis.Y, i2);
               double d5 = shape1.getValueUnchecked(EnumFacing.Axis.Y, i2 + 1);
               for(int j2 = 0; j2 < i1; ++j2) {
                  double d6 = shape2.getValueUnchecked(EnumFacing.Axis.Y, j2);
                  double d7 = shape2.getValueUnchecked(EnumFacing.Axis.Y, j2 + 1);
                  if (d5 <= d6 + 1.0E-7D || d7 <= d4 + 1.0E-7D) {
                     continue;
                  }
                  for(int k2 = 0; k2 < k; ++k2) {
                     if (!shape1.part.contains(k1, i2, k2)) {
                        continue;
                     }
                     double d8 = shape1.getValueUnchecked(EnumFacing.Axis.Z, k2);
                     double d9 = shape1.getValueUnchecked(EnumFacing.Axis.Z, k2 + 1);
                     for(int l2 = 0; l2 < j1; ++l2) {
                        if (shape2.part.contains(l1, j2, l2) && d9 > shape2.getValueUnchecked(EnumFacing.Axis.Z, l2) + 1.0E-7D && shape2.getValueUnchecked(EnumFacing.Axis.Z, l2 + 1) > d8 + 1.0E-7D) {
                           return true;
                        }
                     }
                  }
