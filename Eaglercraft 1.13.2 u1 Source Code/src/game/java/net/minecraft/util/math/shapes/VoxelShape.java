...
public abstract class VoxelShape {
   ...
   public VoxelShape simplify() {
      SimplifyConsumer voxelshape$simplifyconsumer = new SimplifyConsumer();
      this.forEachBox(voxelshape$simplifyconsumer);
      return voxelshape$simplifyconsumer.shape;
   }

   @OnlyIn(Dist.CLIENT)
   public void forEachEdge(VoxelShapes.LineConsumer action) {
      this.part.forEachEdge(new EdgeConsumer(this, action), true);
   }

   public void forEachBox(VoxelShapes.LineConsumer action) {
      this.part.forEachBox(new EdgeConsumer(this, action), true);
   }

   public List<AxisAlignedBB> toBoundingBoxList() {
      List<AxisAlignedBB> list = Lists.newArrayList();
      this.forEachBox(new BoundingBoxConsumer(list));
      return list;
   }

   private static class EdgeConsumer implements VoxelShapePart.LineConsumer {
      private final VoxelShape shape;
      private final VoxelShapes.LineConsumer consumer;

      private EdgeConsumer(VoxelShape shapeIn, VoxelShapes.LineConsumer consumerIn) {
         this.shape = shapeIn;
         this.consumer = consumerIn;
      }

      public void consume(int x1, int y1, int z1, int x2, int y2, int z2) {
         this.consumer.consume(this.shape.getValueUnchecked(EnumFacing.Axis.X, x1), this.shape.getValueUnchecked(EnumFacing.Axis.Y, y1), this.shape.getValueUnchecked(EnumFacing.Axis.Z, z1), this.shape.getValueUnchecked(EnumFacing.Axis.X, x2), this.shape.getValueUnchecked(EnumFacing.Axis.Y, y2), this.shape.getValueUnchecked(EnumFacing.Axis.Z, z2));
      }
   }

   private static class SimplifyConsumer implements VoxelShapes.LineConsumer {
      ...
      public void consume(double x1, double y1, double z1, double x2, double y2, double z2) {
         this.shape = VoxelShapes.combine(this.shape, VoxelShapes.create(x1, y1, z1, x2, y2, z2), IBooleanFunction.OR);
      }
   }

   private static class BoundingBoxConsumer implements VoxelShapes.LineConsumer {
      private final List<AxisAlignedBB> list;

      private BoundingBoxConsumer(List<AxisAlignedBB> listIn) {
         this.list = listIn;
      }

      public void consume(double x1, double y1, double z1, double x2, double y2, double z2) {
         this.list.add(new AxisAlignedBB(x1, y1, z1, x2, y2, z2));
      }
   }
   ...
   protected int getClosestIndex(EnumFacing.Axis axis, double position) {
      int i = 0;
      int j = this.part.getSize(axis) + 1;
      ...
      while(i < j) {
         ...
         if (k < 0) {
            flag = false;
         } else if (k > this.part.getSize(axis)) {
            flag = true;
         } else {
            flag = position < this.getValueUnchecked(axis, k);
         }
   ...
   @Nullable
   public RayTraceResult rayTrace(Vec3d p_212433_1_, Vec3d p_212433_2_, BlockPos p_212433_3_) {
      if (this.isEmpty()) {
         return null;
      } else {
