...
public class BlockSkullWither extends BlockSkull {
   ...
   protected BlockSkullWither(Block.Properties properties) {
   ...
   public static void checkWitherSpawn(World worldIn, BlockPos pos, TileEntitySkull p_196298_2_) {
      ...
      if (flag && pos.getY() >= 2 && worldIn.getDifficulty() != EnumDifficulty.PEACEFUL && !worldIn.isRemote) {
         ...
         if (blockpattern$patternhelper != null) {
            for(int i = 0; i < 3; ++i) {
               TileEntitySkull.disableDrop(worldIn, blockpattern$patternhelper.translateOffset(i, 0, 0).getPos());
            }

            for(int k = 0; k < blockpattern.getPalmLength(); ++k) {
               for(int j = 0; j < blockpattern.getThumbLength(); ++j) {
                  worldIn.setBlockState(blockpattern$patternhelper.translateOffset(k, j, 0).getPos(), Blocks.AIR.getDefaultState(), 2);
               }
            }
            ...
            for(int l = 0; l < 120; ++l) {
               worldIn.addParticle(Particles.ITEM_SNOWBALL, (double)blockpos1.getX() + worldIn.rand.nextDouble(), (double)(blockpos1.getY() - 2) + worldIn.rand.nextDouble() * 3.9D, (double)blockpos1.getZ() + worldIn.rand.nextDouble(), 0.0D, 0.0D, 0.0D);
            }

            for(int i1 = 0; i1 < blockpattern.getPalmLength(); ++i1) {
               for(int j1 = 0; j1 < blockpattern.getThumbLength(); ++j1) {
                  worldIn.notifyNeighbors(blockpattern$patternhelper.translateOffset(i1, j1, 0).getPos(), Blocks.AIR);
               }
            }
