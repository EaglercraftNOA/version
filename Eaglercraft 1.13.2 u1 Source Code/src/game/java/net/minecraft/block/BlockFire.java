...
public class BlockFire extends Block {
   ...
   private static final Map<EnumFacing, BooleanProperty> FACING_TO_PROPERTY_MAP = BlockSixWay.FACING_TO_PROPERTY_MAP.entrySet().stream().filter((p_199776_0_) -> {
   ...
   public IBlockState getStateForPlacement(IBlockReader p_196448_1_, BlockPos p_196448_2_) {
      ...
      if (!iblockstate.isTopSolid() && !this.canBurn(iblockstate)) {
         ...
         for(EnumFacing enumfacing : EnumFacing._VALUES) {
            ...
            if (booleanproperty != null) {
               iblockstate1 = iblockstate1.with(booleanproperty, Boolean.valueOf(this.canBurn(p_196448_1_.getBlockState(p_196448_2_.offset(enumfacing)))));
            }
         }
   ...
   public boolean isValidPosition(IBlockState state, IWorldReaderBase worldIn, BlockPos pos) {
      return worldIn.getBlockState(pos.down()).isTopSolid() || this.areNeighborsFlammable(worldIn, pos);
   }
   ...
   public void animateTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
      ...
      if (!worldIn.getBlockState(pos.down()).isTopSolid() && !this.canBurn(worldIn.getBlockState(pos.down()))) {
         if (this.canBurn(worldIn.getBlockState(pos.west()))) {
            for(int j = 0; j < 2; ++j) {
               double d3 = (double)pos.getX() + rand.nextDouble() * (double)0.1F;
               double d8 = (double)pos.getY() + rand.nextDouble();
               double d13 = (double)pos.getZ() + rand.nextDouble();
               worldIn.addParticle(Particles.LARGE_SMOKE, d3, d8, d13, 0.0D, 0.0D, 0.0D);
            }
         }
         ...
         if (this.canBurn(worldIn.getBlockState(pos.east()))) {
            for(int k = 0; k < 2; ++k) {
               ...
               worldIn.addParticle(Particles.LARGE_SMOKE, d4, d9, d14, 0.0D, 0.0D, 0.0D);
            }
         }
         ...
         if (this.canBurn(worldIn.getBlockState(pos.north()))) {
            for(int l = 0; l < 2; ++l) {
               ...
               worldIn.addParticle(Particles.LARGE_SMOKE, d5, d10, d15, 0.0D, 0.0D, 0.0D);
            }
         }
         ...
         if (this.canBurn(worldIn.getBlockState(pos.south()))) {
            for(int i1 = 0; i1 < 2; ++i1) {
               ...
               worldIn.addParticle(Particles.LARGE_SMOKE, d6, d11, d16, 0.0D, 0.0D, 0.0D);
            }
         }
         ...
         if (this.canBurn(worldIn.getBlockState(pos.up()))) {
            for(int j1 = 0; j1 < 2; ++j1) {
               ...
               worldIn.addParticle(Particles.LARGE_SMOKE, d7, d12, d17, 0.0D, 0.0D, 0.0D);
            }
         }
      } else {
         for(int i = 0; i < 3; ++i) {
            ...
            worldIn.addParticle(Particles.LARGE_SMOKE, d0, d1, d2, 0.0D, 0.0D, 0.0D);
         }
      }
