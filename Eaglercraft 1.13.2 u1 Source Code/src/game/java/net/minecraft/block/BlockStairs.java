...
public class BlockStairs extends Block implements IBucketPickupHandler, ILiquidContainer {
   ...
   protected BlockStairs(IBlockState state, Block.Properties properties) {
      super(properties);
   ...
   private static StairsShape func_208064_n(IBlockState state, IBlockReader worldIn, BlockPos pos) {
      EnumFacing enumfacing = state.get(FACING);
      ...
      if (isBlockStairs(iblockstate) && state.get(HALF) == iblockstate.get(HALF)) {
         ...
         if (enumfacing1.getAxis() != state.get(FACING).getAxis() && isDifferentStairs(state, worldIn, pos, enumfacing1.getOpposite())) {
      ...
      if (isBlockStairs(iblockstate1) && state.get(HALF) == iblockstate1.get(HALF)) {
         ...
         if (enumfacing2.getAxis() != state.get(FACING).getAxis() && isDifferentStairs(state, worldIn, pos, enumfacing2)) {
   ...
   private static boolean isDifferentStairs(IBlockState state, IBlockReader worldIn, BlockPos pos, EnumFacing face) {
