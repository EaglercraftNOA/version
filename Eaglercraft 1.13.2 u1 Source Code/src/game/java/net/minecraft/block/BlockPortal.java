...
public class BlockPortal extends Block {
   ...
   public void tick(IBlockState state, World worldIn, BlockPos pos, Random random) {
      if (worldIn.dimension.isSurfaceWorld() && worldIn.getGameRules().getBoolean("doMobSpawning") && random.nextInt(2000) < worldIn.getDifficulty().getId()) {
         ...
         if (i > 0 && !worldIn.getBlockState(blockpos.up()).isNormalCube()) {
            Entity entity = EntityType.ZOMBIE_PIGMAN.spawn(worldIn, (NBTTagCompound)null, (ITextComponent)null, (EntityPlayer)null, blockpos.up(), false, false);
            if (entity != null) {
   ...
   public BlockPattern.PatternHelper createPatternHelper(IWorld worldIn, BlockPos p_181089_2_) {
      ...
      BlockPattern.BlockCache loadingcache = BlockPattern.createLoadingCache(worldIn, true);
