...
public class AnvilSaveConverter extends SaveFormatOld {
   ...
   public AnvilSaveConverter(VFile2 p_i49567_1_, VFile2 p_i49567_2_, DataFixer p_i49567_3_) {
   ...
   public List<WorldSummary> getSaveList() throws AnvilConverterException {
      if (!this.savesDirectory.exists()) {
         return Lists.newArrayList();
      } else {
         List<WorldSummary> list = Lists.newArrayList();
         List<VFile2> afile = this.savesDirectory.listFiles(false);
   ...
   public ISaveHandler getSaveLoader(String saveName, @Nullable MinecraftServer server) {
      return new AnvilSaveHandler(this.savesDirectory, saveName, server, this.dataFixer);
   }
   ...
   public boolean convertMapFormat(String filename, IProgressUpdate progressCallback) {
      ...
      VFile2 file1 = WorldsDB.newVFile(this.savesDirectory, filename);
      VFile2 file2 = DimensionType.NETHER.getDirectory(file1);
      ...
      this.convertFile(WorldsDB.newVFile(file1, "region"), list, biomeprovider, 0, i, progressCallback);
      this.convertFile(WorldsDB.newVFile(file2, "region"), list1, biomeprovidertype.create(biomeprovidertype.createSettings().setBiome(Biomes.NETHER)), list.size(), i, progressCallback);
      this.convertFile(WorldsDB.newVFile(file3, "region"), list2, biomeprovidertype.create(biomeprovidertype.createSettings().setBiome(Biomes.THE_END)), list.size() + list1.size(), i, progressCallback);
   ...
   private void createFile(String filename) {
      ...
      if (!file1.exists()) {
         ...
         if (!file2.exists()) {
            ...
            VFile2 file3 = WorldsDB.newVFile(file1, "level.dat_mcr");
   ...
   private void convertFile(VFile2 baseFolder, Iterable<VFile2> regionFiles, BiomeProvider p_75813_3_, int p_75813_4_, int p_75813_5_, IProgressUpdate progress) {
      for(VFile2 file1 : regionFiles) {
   ...
   private void convertChunks(VFile2 baseFolder, VFile2 p_75811_2_, BiomeProvider biomeSource, int p_75811_4_, int p_75811_5_, IProgressUpdate progressCallback) {
      try {
         String s = p_75811_2_.getName();
         RegionFile regionfile = new RegionFile(p_75811_2_);
         RegionFile regionfile1 = new RegionFile(WorldsDB.newVFile(baseFolder, s.substring(0, s.length() - ".mcr".length()) + ".mca"));
   ...
   private void addRegionFilesToCollection(VFile2 worldDir, Collection<VFile2> collection) {
