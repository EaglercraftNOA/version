...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
   ...
   public AnvilChunkLoader(VFile2 chunkSaveLocationIn, DataFixer fixerIn) {
   ...
   private NBTTagCompound func_212146_a(DimensionType p_212146_1_, @Nullable WorldSavedDataStorage storage, int chunkX, int chunkZ) throws IOException {
      ...
      if (nbttagcompound != null) {
         ...
         if (datainputstream == null) {
            ...
            if (i < 1631) {
               nbttagcompound1.putInt("DataVersion", 1631);
   ...
   private void func_202156_a(ChunkPrimer primer, World worldIn, NBTTagCompound compound) {
      ...
      compound.putInt("zPos", j);
      compound.putLong("LastUpdate", worldIn.getGameTime());
      compound.putLong("InhabitedTime", primer.getInhabitedTime());
      compound.putString("Status", primer.getStatus().getName());
      ...
      if (abiome != null) {
         for(int k = 0; k < abiome.length; ++k) {
            aint[k] = IRegistry.BIOME.getId(abiome[k]);
         }
      }

      compound.putIntArray("Biomes", aint);
      ...
      compound.put("Lights", listArrayToTag(primer.getPackedLightPositions()));
      compound.put("PostProcessing", listArrayToTag(primer.getPackedPositions()));
      compound.put("ToBeTicked", primer.getBlocksToBeTicked().write());
      compound.put("LiquidsToBeTicked", primer.getFluidsToBeTicked().write());
      NBTTagCompound nbttagcompound2 = new NBTTagCompound();

      for(Heightmap.Type heightmap$type : primer.getHeightMapKeys()) {
         nbttagcompound2.put(heightmap$type.getId(), new NBTTagLongArray(primer.getHeightmap(heightmap$type).getDataArray()));
      }

      compound.put("Heightmaps", nbttagcompound2);
      NBTTagCompound nbttagcompound3 = new NBTTagCompound();

      for(GenerationStage.Carving generationstage$carving : GenerationStage.Carving.values()) {
         nbttagcompound3.putByteArray(generationstage$carving.toString(), primer.getCarvingMask(generationstage$carving).toByteArray());
      }

      compound.put("CarvingMasks", nbttagcompound3);
      compound.put("Structures", this.createStructuresTag(i, j, primer.getStructureStarts(), primer.getStructureReferences()));
   }
   ...
   private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound compound) {
      ...
      compound.putString("Status", chunkIn.getStatus().getName());
      ...
      for(int i = 0; i < abiome.length; ++i) {
         aint[i] = IRegistry.BIOME.getId(abiome[i]);
      }
      ...
      for(BlockPos blockpos : chunkIn.getTileEntitiesPos()) {
         ...
         if (tileentity != null) {
            NBTTagCompound nbttagcompound1 = new NBTTagCompound();
            tileentity.write(nbttagcompound1);
            nbttagcompound1.putBoolean("keepPacked", false);
            nbttaglist2.add((INBTBase)nbttagcompound1);
         } else {
            NBTTagCompound nbttagcompound3 = chunkIn.getDeferredTileEntity(blockpos);
            if (nbttagcompound3 != null) {
               nbttagcompound3.putBoolean("keepPacked", true);
      ...
      if (worldIn.getPendingBlockTicks() instanceof ServerTickList) {
         compound.put("TileTicks", ((ServerTickList)worldIn.getPendingBlockTicks()).write(chunkIn));
      }

      if (worldIn.getPendingFluidTicks() instanceof ServerTickList) {
         compound.put("LiquidTicks", ((ServerTickList)worldIn.getPendingFluidTicks()).write(chunkIn));
      }

      compound.put("PostProcessing", listArrayToTag(chunkIn.getPackedPositions()));
      if (chunkIn.getBlocksToBeTicked() instanceof ChunkPrimerTickList) {
         compound.put("ToBeTicked", ((ChunkPrimerTickList)chunkIn.getBlocksToBeTicked()).write());
      }

      if (chunkIn.getFluidsToBeTicked() instanceof ChunkPrimerTickList) {
         compound.put("LiquidsToBeTicked", ((ChunkPrimerTickList)chunkIn.getFluidsToBeTicked()).write());
      }
      ...
      for(Heightmap.Type heightmap$type : chunkIn.getHeightmaps()) {
         if (heightmap$type.getUsage() == Heightmap.Usage.LIVE_WORLD) {
            nbttagcompound2.put(heightmap$type.getId(), new NBTTagLongArray(chunkIn.getHeightmap(heightmap$type).getDataArray()));
         }
      }

      compound.put("Heightmaps", nbttagcompound2);
      compound.put("Structures", this.createStructuresTag(chunkIn.x, chunkIn.z, chunkIn.getStructureStarts(), chunkIn.getStructureReferences()));
   }
   ...
   private Chunk readChunkFromNBT(IWorld worldIn, NBTTagCompound compound) {
      ...
      if (compound.contains("Biomes", 11)) {
         ...
         for(int k = 0; k < aint.length; ++k) {
            abiome[k] = IRegistry.BIOME.get(aint[k]);
            if (abiome[k] == null) {
      ...
      }, IRegistry.BLOCK::getKey, IRegistry.BLOCK::getOrDefault, new ChunkPos(i, j));
      ChunkPrimerTickList<Fluid> chunkprimerticklist = new ChunkPrimerTickList<>((p_206242_0_) -> {
         return p_206242_0_ == Fluids.EMPTY;
      }, IRegistry.FLUID::getKey, IRegistry.FLUID::getOrDefault, new ChunkPos(i, j));
      ...
      chunkprimerticklist1.read(compound.getList("ToBeTicked", 9));
      chunkprimerticklist.read(compound.getList("LiquidsToBeTicked", 9));
   ...
   private ChunkPrimer readChunkPrimerFromNBT(IWorld worldIn, NBTTagCompound nbt) {
      ...
      chunkprimer.getBlocksToBeTicked().read(nbt.getList("ToBeTicked", 9));
      chunkprimer.getFluidsToBeTicked().read(nbt.getList("LiquidsToBeTicked", 9));
   ...
   private NBTTagCompound createStructuresTag(int chunkX, int chunkZ, Map<String, StructureStart> structureStarts, Map<String, LongSet> structureReferences) {
      ...
      for(Entry<String, StructureStart> entry : structureStarts.entrySet()) {
         nbttagcompound1.put(entry.getKey(), entry.getValue().write(chunkX, chunkZ));
      }
      ...
      for(Entry<String, LongSet> entry1 : structureReferences.entrySet()) {
         nbttagcompound2.put(entry1.getKey(), new NBTTagLongArray(entry1.getValue()));
      }
