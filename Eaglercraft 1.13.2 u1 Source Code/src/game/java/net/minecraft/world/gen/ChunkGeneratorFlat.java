...
public class ChunkGeneratorFlat extends AbstractChunkGenerator<FlatGenSettings> {
   ...
   private Biome func_202099_e() {
      ...
      for(String s : map.keySet()) {
         CompositeFeature<?, ?>[] compositefeature = FlatGenSettings.STRUCTURES.get(s);
         if (compositefeature != null) {
            for(CompositeFeature<?, ?> compositefeature1 : compositefeature) {
               chunkgeneratorflat$biomewrapper.addFeature(FlatGenSettings.FEATURE_STAGES.get(compositefeature1), compositefeature1);
               ...
               if (feature instanceof Structure) {
                  ...
                  chunkgeneratorflat$biomewrapper.addStructure((Structure)feature, ifeatureconfig != null ? ifeatureconfig : FlatGenSettings.FEATURE_CONFIGS.get(compositefeature1));
               }
            }
         }
      }
      ...
      for(int l = j - 8; l <= j + 8; ++l) {
         for(int i1 = k - 8; i1 <= k + 8; ++i1) {
            ...
            while(listiterator.hasNext()) {
               ...
               if (worldcarverwrapper.shouldCarve(region, sharedseedrandom, l, i1, IFeatureConfig.NO_FEATURE_CONFIG)) {
