...
public class CanyonWorldCarver extends WorldCarver<ProbabilityConfig> {
   ...
   public boolean shouldCarve(IBlockReader blockReader, Random rand, int x, int z, ProbabilityConfig config) {
                              ...
                              if (this.isTargetSafeFromFalling(iblockstate, iblockstate1)) {
                                 if (k2 - 1 < 10) {
                                    ...
                                    if (flag1 && worldIn.getBlockState(blockpos$mutableblockpos2).getBlock() == Blocks.DIRT) {
                                       worldIn.setBlockState(blockpos$mutableblockpos2, worldIn.getBiome(blockpos$mutableblockpos).getSurfaceBuilderConfig().getTopMaterial(), 2);
                                    }
                                 }
