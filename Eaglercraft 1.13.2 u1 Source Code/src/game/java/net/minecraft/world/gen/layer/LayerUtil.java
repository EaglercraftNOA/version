...
public class LayerUtil {
   ...
   private static <T extends IArea, C extends IContextExtended<T>> IAreaFactory<T> repeat(long seed, IAreaTransformer1 parent, IAreaFactory<T> p_202829_3_, int count, LongFunction<C> contextFactory) {
   ...
   public static <T extends IArea, C extends IContextExtended<T>> ImmutableList<IAreaFactory<T>> buildOverworldProcedure(WorldType worldTypeIn, OverworldGenSettings settings, LongFunction<C> contextFactory) {
      ...
      IAreaFactory<T> lvt_8_1_ = (new GenLayerBiome(worldTypeIn, settings)).apply(contextFactory.apply(200L), iareafactory);
      lvt_8_1_ = repeat(1000L, GenLayerZoom.NORMAL, lvt_8_1_, 2, contextFactory);
      lvt_8_1_ = GenLayerBiomeEdge.INSTANCE.apply((IContextExtended)contextFactory.apply(1000L), lvt_8_1_);
   ...
   public static boolean areBiomesSimilar(int p_202826_0_, int p_202826_1_) {
      if (p_202826_0_ == p_202826_1_) {
         ...
         Biome biome = IRegistry.BIOME.get(p_202826_0_);
         Biome biome1 = IRegistry.BIOME.get(p_202826_1_);
