...
public class WorldGenRegion implements IWorld {
   ...
   private final ITickList<Fluid> pendingFluidTickList = new WorldGenTickList<>((p_205334_1_) -> {
      return this.getChunkDefault(p_205334_1_).getFluidsToBeTicked();
   });
      ...
      if (tileentity != null) {
         ...
         if (nbttagcompound != null) {
            if ("DUMMY".equals(nbttagcompound.getString("id"))) {
               tileentity = ((ITileEntityProvider)this.getBlockState(pos).getBlock()).createNewTileEntity(this.world);
            } else {
   ...
   public int getHeight(Heightmap.Type heightmapType, int x, int z) {
      int i = x >> 4;
      int j = z >> 4;
      if (this.isChunkInBounds(i, j)) {
         return this.getChunk(i, j).getTopBlockY(heightmapType, x & 15, z & 15) + 1;
      } else {
         IChunk ichunk = this.chunkPrimers[0];
         IChunk ichunk1 = this.chunkPrimers[this.chunkPrimers.length - 1];
         int k = MathHelper.clamp(i, ichunk.getPos().x, ichunk1.getPos().x);
         int l = MathHelper.clamp(j, ichunk.getPos().z, ichunk1.getPos().z);
         return this.getChunk(k, l).getTopBlockY(heightmapType, x & 15, z & 15) + 1;
      }
   }
