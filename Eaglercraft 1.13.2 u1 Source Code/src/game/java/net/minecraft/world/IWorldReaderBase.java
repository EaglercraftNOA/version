...
public interface IWorldReaderBase extends IBlockReader {
      ...
      for(int k1 = i; k1 < j; ++k1) {
         for(int l1 = k; l1 < l; ++l1) {
            for(int i2 = i1; i2 < j1; ++i2) {
               ...
               if (!this.isBlockLoaded(blockpos$pooledmutableblockpos)) {
                  continue;
               }

               VoxelShape voxelshape;
               if (p_212391_3_ && !flag && !worldborder.contains(blockpos$pooledmutableblockpos)) {
                  voxelshape = VoxelShapes.fullCube();
               } else {
                  voxelshape = this.getBlockState(blockpos$pooledmutableblockpos).getCollisionShape(this, blockpos$pooledmutableblockpos);
               }
               ...
               VoxelShape voxelshape2 = voxelshape.withOffset((double)k1, (double)l1, (double)i2);
               if (!voxelshape2.isEmpty() && VoxelShapes.compare(p_212391_1_, voxelshape2, IBooleanFunction.AND)) {
                  list.add(voxelshape2);
               }
            }
         }
      }
   ...
   default Stream<VoxelShape> func_212389_a(@Nullable Entity p_212389_1_, AxisAlignedBB p_212389_2_, Set<Entity> entitiesToIgnore, double dx, double dy, double dz) {
      ...
      VoxelShape voxelshape2 = VoxelShapes.create(p_212389_2_.expand(dx, dy, dz).grow(1.0E-7D));
   ...
   default boolean isCollisionBoxesEmpty(@Nullable Entity entityIn, AxisAlignedBB aabb, Set<Entity> entitiesToIgnore) {
      Iterator<VoxelShape> iterator = this.getCollisionBoxes(entityIn, VoxelShapes.create(aabb), VoxelShapes.empty(), entitiesToIgnore).iterator();
