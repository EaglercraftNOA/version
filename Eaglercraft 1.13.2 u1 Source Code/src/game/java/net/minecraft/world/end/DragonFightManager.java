...
public class DragonFightManager {
   ...
   public DragonFightManager(WorldServer worldIn, NBTTagCompound compound) {
      ...
      if (compound.contains("Gateways", 9)) {
         ...
         for(int i = this.gateways.size() - 1; i > 0; --i) {
            int j = random.nextInt(i + 1);
            int k = this.gateways.get(i);
            this.gateways.set(i, this.gateways.get(j));
            this.gateways.set(j, k);
         }
      }
   ...
   private void generateGateway(BlockPos pos) {
      ...
      Feature.END_GATEWAY.place(this.world, this.world.getChunkProvider().getChunkGenerator(), new Random(), pos, new EndGatewayConfig(false));
   }
