               ...
               for(ChunkSection chunksection : chunk.getSections()) {
                  if (chunksection != Chunk.EMPTY_SECTION && chunksection.needsRandomTickAny()) {
                     for(int j2 = 0; j2 < i; ++j2) {
                        ...
                        if (iblockstate.ticksRandomly() || ifluidstate.ticksRandomly()) {
                           BlockPos blockpos = new BlockPos(j1 + j, l1 + chunksection.getYLocation(), k1 + k);
                           if (iblockstate.ticksRandomly()) {
                              iblockstate.randomTick(this, blockpos, this.rand);
                           }

                           if (ifluidstate.ticksRandomly()) {
                              ifluidstate.randomTick(this, blockpos, this.rand);
                           }
                        }
