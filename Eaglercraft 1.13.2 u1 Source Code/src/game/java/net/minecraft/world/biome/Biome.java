...
public abstract class Biome {
   ...
   public boolean doesSnowGenerate(IWorldReaderBase worldIn, BlockPos pos) {
      if (this.getTemperature(pos) >= 0.15F) {
         ...
         if (pos.getY() >= 0 && pos.getY() < 256 && worldIn.getLightFor(EnumLightType.BLOCK, pos) < 10) {
            ...
            if (iblockstate.isAir() && Blocks.SNOW.getDefaultState().isValidPosition(worldIn, pos)) {
               return true;
            }
         }
   ...
   public void decorate(GenerationStage.Decoration stage, IChunkGenerator<? extends IChunkGenSettings> chunkGenerator, IWorld worldIn, long p_203608_4_, SharedSeedRandom random, BlockPos pos) {
      ...
      for(CompositeFeature<?, ?> compositefeature : this.features.get(stage)) {
         ...
         compositefeature.place(worldIn, chunkGenerator, random, pos, IFeatureConfig.NO_FEATURE_CONFIG);
         ++i;
      }
   ...
   private static void register(int id, String name, Biome biome) {
      IRegistry.BIOME.register(id, new ResourceLocation(name), biome);
      if (biome.isMutation()) {
         MUTATION_TO_BASE_ID_MAP.put(biome, IRegistry.BIOME.getId(IRegistry.BIOME.get(new ResourceLocation(biome.parent))));
      }
