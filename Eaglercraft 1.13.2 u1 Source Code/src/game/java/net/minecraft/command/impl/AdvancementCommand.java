...
public class AdvancementCommand {
   ...
   private static int forEachAdvancement(CommandSource source, Collection<EntityPlayerMP> targets, AdvancementCommand.Action action, Collection<Advancement> advancements) {
      ...
      if (i == 0) {
         if (advancements.size() == 1) {
            if (targets.size() == 1) {
               throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".one.to.one.failure", advancements.iterator().next().getDisplayText(), targets.iterator().next().getDisplayName()));
            } else {
               throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".one.to.many.failure", advancements.iterator().next().getDisplayText(), targets.size()));
            }
         } else if (targets.size() == 1) {
            throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".many.to.one.failure", advancements.size(), targets.iterator().next().getDisplayName()));
         } else {
            throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".many.to.many.failure", advancements.size(), targets.size()));
         }
      } else {
         if (advancements.size() == 1) {
            if (targets.size() == 1) {
               source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".one.to.one.success", advancements.iterator().next().getDisplayText(), targets.iterator().next().getDisplayName()), true);
            } else {
               source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".one.to.many.success", advancements.iterator().next().getDisplayText(), targets.size()), true);
            }
         } else if (targets.size() == 1) {
            source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".many.to.one.success", advancements.size(), targets.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".many.to.many.success", advancements.size(), targets.size()), true);
         }
   ...
   private static int updateCriterion(CommandSource source, Collection<EntityPlayerMP> targets, AdvancementCommand.Action action, Advancement advancementIn, String criterionName) {
      ...
      if (!advancementIn.getCriteria().containsKey(criterionName)) {
         throw new CommandException(new TextComponentTranslation("commands.advancement.criterionNotFound", advancementIn.getDisplayText(), criterionName));
      } else {
         for(EntityPlayerMP entityplayermp : targets) {
         ...
         if (i == 0) {
            if (targets.size() == 1) {
               throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".criterion.to.one.failure", criterionName, advancementIn.getDisplayText(), targets.iterator().next().getDisplayName()));
            } else {
               throw new CommandException(new TextComponentTranslation(action.getPrefix() + ".criterion.to.many.failure", criterionName, advancementIn.getDisplayText(), targets.size()));
            }
         } else {
            if (targets.size() == 1) {
               source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".criterion.to.one.success", criterionName, advancementIn.getDisplayText(), targets.iterator().next().getDisplayName()), true);
            } else {
               source.sendFeedback(new TextComponentTranslation(action.getPrefix() + ".criterion.to.many.success", criterionName, advancementIn.getDisplayText(), targets.size()), true);
            }
