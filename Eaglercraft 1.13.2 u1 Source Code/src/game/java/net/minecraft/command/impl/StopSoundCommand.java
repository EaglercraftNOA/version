...
public class StopSoundCommand {
   ...
   private static int stopSound(CommandSource source, Collection<EntityPlayerMP> targets, @Nullable SoundCategory category, @Nullable ResourceLocation soundIn) {
      ...
      if (category != null) {
         if (soundIn != null) {
            source.sendFeedback(new TextComponentTranslation("commands.stopsound.success.source.sound", soundIn, category.getName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.stopsound.success.source.any", category.getName()), true);
         }
      } else if (soundIn != null) {
         source.sendFeedback(new TextComponentTranslation("commands.stopsound.success.sourceless.sound", soundIn), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.stopsound.success.sourceless.any"), true);
      }
