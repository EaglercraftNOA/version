...
public class SpreadPlayersCommand {
   ...
   private static int spreadPlayers(CommandSource source, Vec2f center, float spreadDistance, float maxRange, boolean respectTeams, Collection<? extends Entity> targets) throws CommandSyntaxException {
      ...
      source.sendFeedback(new TextComponentTranslation("commands.spreadplayers.success." + (respectTeams ? "teams" : "entities"), aspreadplayerscommand$position.length, center.x, center.y, String.format(Locale.ROOT, "%.2f", d4)), true);
   ...
   private static void ensureSufficientSeparation(Vec2f center, double spreadDistance, WorldServer worldIn, Random random, double minX, double minZ, double maxX, double maxZ, SpreadPlayersCommand.Position[] positions, boolean respectTeams) throws CommandSyntaxException {
      boolean flag = true;
   ...
   private static double doSpreading(Collection<? extends Entity> targets, WorldServer worldIn, SpreadPlayersCommand.Position[] positions, boolean respectTeams) {
      double d0 = 0.0D;
      int i = 0;
      ...
      for(Entity entity : targets) {
         ...
         entity.setPositionAndUpdate((double)((float)MathHelper.floor(spreadplayerscommand$position.x) + 0.5F), (double)spreadplayerscommand$position.getHighestNonAirBlock(worldIn), (double)MathHelper.floor(spreadplayerscommand$position.z) + 0.5D);
