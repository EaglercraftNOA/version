...
public class MessageCommand {
   ...
   private static int sendPrivateMessage(CommandSource source, Collection<EntityPlayerMP> recipients, ITextComponent message) {
      for(EntityPlayerMP entityplayermp : recipients) {
         entityplayermp.sendMessage((new TextComponentTranslation("commands.message.display.incoming", source.getDisplayName(), message.deepCopy())).applyTextStyles(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}));
         source.sendFeedback((new TextComponentTranslation("commands.message.display.outgoing", entityplayermp.getDisplayName(), message.deepCopy())).applyTextStyles(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}), false);
      }
