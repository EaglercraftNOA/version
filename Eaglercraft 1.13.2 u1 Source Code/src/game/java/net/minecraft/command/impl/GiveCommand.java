...
public class GiveCommand {
   ...
   private static int giveItem(CommandSource source, ItemInput itemIn, Collection<EntityPlayerMP> targets, int count) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(new TextComponentTranslation("commands.give.success.single", count, itemIn.createStack(count, false).getTextComponent(), targets.iterator().next().getDisplayName()), true);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.give.success.single", count, itemIn.createStack(count, false).getTextComponent(), targets.size()), true);
      }
