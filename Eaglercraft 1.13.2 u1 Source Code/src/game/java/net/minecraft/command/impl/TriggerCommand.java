...
public class TriggerCommand {
   private static final SimpleCommandExceptionType NOT_PRIMED = new SimpleCommandExceptionType(new TextComponentTranslation("commands.trigger.failed.unprimed"));
   private static final SimpleCommandExceptionType NOT_A_TRIGGER = new SimpleCommandExceptionType(new TextComponentTranslation("commands.trigger.failed.invalid"));
   ...
   private static int addToTrigger(CommandSource source, Score objective, int amount) {
      ...
      source.sendFeedback(new TextComponentTranslation("commands.trigger.add.success", objective.getObjective().func_197890_e(), amount), true);
   ...
   private static int setTrigger(CommandSource source, Score objective, int value) {
      ...
      source.sendFeedback(new TextComponentTranslation("commands.trigger.set.success", objective.getObjective().func_197890_e(), value), true);
      return value;
   }

   private static int incrementTrigger(CommandSource source, Score objectives) {
      objectives.increaseScore(1);
      source.sendFeedback(new TextComponentTranslation("commands.trigger.simple.success", objectives.getObjective().func_197890_e()), true);
