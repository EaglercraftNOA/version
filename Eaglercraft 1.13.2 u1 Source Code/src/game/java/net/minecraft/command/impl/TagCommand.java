...
public class TagCommand {
   private static final SimpleCommandExceptionType ADD_FAILED = new SimpleCommandExceptionType(new TextComponentTranslation("commands.tag.add.failed"));
   private static final SimpleCommandExceptionType REMOVE_FAILED = new SimpleCommandExceptionType(new TextComponentTranslation("commands.tag.remove.failed"));
   ...
   private static int addTag(CommandSource source, Collection<? extends Entity> entities, String tagName) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (entities.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.tag.add.success.single", tagName, entities.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.tag.add.success.multiple", tagName, entities.size()), true);
         }
   ...
   private static int removeTag(CommandSource source, Collection<? extends Entity> entities, String tagName) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         if (entities.size() == 1) {
            source.sendFeedback(new TextComponentTranslation("commands.tag.remove.success.single", tagName, entities.iterator().next().getDisplayName()), true);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.tag.remove.success.multiple", tagName, entities.size()), true);
         }
   ...
   private static int listTags(CommandSource source, Collection<? extends Entity> entities) {
      ...
      if (entities.size() == 1) {
         ...
         if (set.isEmpty()) {
            source.sendFeedback(new TextComponentTranslation("commands.tag.list.single.empty", entity1.getDisplayName()), false);
         } else {
            source.sendFeedback(new TextComponentTranslation("commands.tag.list.single.success", entity1.getDisplayName(), set.size(), TextComponentUtils.makeGreenSortedList(set)), false);
         }
      } else if (set.isEmpty()) {
         source.sendFeedback(new TextComponentTranslation("commands.tag.list.multiple.empty", entities.size()), false);
      } else {
         source.sendFeedback(new TextComponentTranslation("commands.tag.list.multiple.success", entities.size(), set.size(), TextComponentUtils.makeGreenSortedList(set)), false);
      }
