   ...
   private void load() {
      if (this.progressFile.isFile()) {
         try (JsonReader jsonreader = new JsonReader(new StringReader(Files.toString(this.progressFile, StandardCharsets.UTF_8)))) {
            ...
            JsonElement advancementsElement = dynamic.getValue();
            if (advancementsElement == null || !advancementsElement.isJsonObject()) {
               throw new JsonParseException("Found null for advancements");
            }

            Map<ResourceLocation, AdvancementProgress> map = Maps.newHashMap();

            for(Entry<String, JsonElement> progressEntry : advancementsElement.getAsJsonObject().entrySet()) {
               map.put(new ResourceLocation(progressEntry.getKey()), GSON.fromJson(progressEntry.getValue(), AdvancementProgress.class));
            }
