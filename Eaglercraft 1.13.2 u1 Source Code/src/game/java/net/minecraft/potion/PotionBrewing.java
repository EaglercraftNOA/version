...
public class PotionBrewing {
   ...
   protected static boolean hasTypeConversions(ItemStack input, ItemStack reagent) {
      ...
      for(int j = POTION_TYPE_CONVERSIONS.size(); i < j; ++i) {
         ...
         if (mixpredicate.input == potiontype && mixpredicate.reagent.test(reagent)) {
            return true;
         }
      }
   ...
   public static ItemStack doReaction(ItemStack reagent, ItemStack potionIn) {
      if (!potionIn.isEmpty()) {
         ...
         for(int k = POTION_TYPE_CONVERSIONS.size(); i < k; ++i) {
            ...
            if (mixpredicate1.input == potiontype && mixpredicate1.reagent.test(reagent)) {
   ...
   private static void addContainerRecipe(Item p_196207_0_, Item p_196207_1_, Item p_196207_2_) {
      if (!(p_196207_0_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.ITEM.getKey(p_196207_0_));
      } else if (!(p_196207_2_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.ITEM.getKey(p_196207_2_));
      } else {
   ...
   private static void addContainer(Item p_196208_0_) {
      if (!(p_196208_0_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.ITEM.getKey(p_196208_0_));
      } else {
