...
public abstract class MobSpawnerBaseLogic {
   ...
   public void setEntityType(EntityType<?> type) {
      this.spawnData.getNbt().putString("id", IRegistry.ENTITY_TYPE.getKey(type).toString());
   }

   private boolean isActivated() {
      BlockPos blockpos = this.getSpawnerPosition();
      return this.getWorld().isLivingPlayerWithinRange((double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.5D, (double)blockpos.getZ() + 0.5D, (double)this.activatingRangeFromPlayer);
   }
   ...
   public void tick() {
      if (!this.isActivated()) {
         ...
         if (this.getWorld().isRemote) {
            ...
            this.getWorld().addParticle(Particles.SMOKE, d3, d4, d5, 0.0D, 0.0D, 0.0D);
            this.getWorld().addParticle(Particles.FLAME, d3, d4, d5, 0.0D, 0.0D, 0.0D);
            ...
            for(int i = 0; i < this.spawnCount; ++i) {
               ...
               if (entityliving == null || entityliving.canSpawn(world, true) && entityliving.isNotColliding()) {
