...
public class TileEntityEndGateway extends TileEntityEndPortal implements ITickable {
   ...
   private void findExitPortal() {
      ...
      if (this.exitPortal == null) {
         ...
         (new EndIslandFeature()).place(this.world, this.world.getChunkProvider().getChunkGenerator(), new Random(this.exitPortal.toLong()), this.exitPortal, IFeatureConfig.NO_FEATURE_CONFIG);
      } else {
   ...
   private void createExitPortal(BlockPos posIn) {
      Feature.END_GATEWAY.place(this.world, this.world.getChunkProvider().getChunkGenerator(), new Random(), posIn, new EndGatewayConfig(false));
