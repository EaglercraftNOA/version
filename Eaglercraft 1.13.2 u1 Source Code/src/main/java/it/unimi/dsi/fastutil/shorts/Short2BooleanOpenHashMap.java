...
public class Short2BooleanOpenHashMap extends AbstractShort2BooleanMap implements java.io.Serializable, Cloneable, Hash {
	...
	private int find(final short k) {
		...
		int pos = (it.unimi.dsi.fastutil.HashCommon.mix((k))) & mask;
		// The starting point.
		curr = key[pos];
		if (((curr) == ((short)0))) return -(pos + 1);
		if (((k) == (curr))) {
			return pos;
		}
	...
	public boolean remove(final short k) {
		...
		// The starting point.
		curr = key[pos];
		if (((curr) == ((short)0))) return defRetValue;
		if (((k) == (curr))) {
	...
	public boolean getOrDefault(final short k, final boolean defaultValue) {
		...
		// The starting point.
		curr = key[pos];
		if (((curr) == ((short)0))) return defaultValue;
		if (((k) == (curr))) {
			boolean result = value[pos];
			return result;
		}
