...
final class FixedCompositeByteBuf extends AbstractReferenceCountedByteBuf {
    ...
    private ITextComponent findComponent(int index) {
        int readable = 0;
        for (int i = 0 ; i < buffers.length; i++) {
            ...
            if (index < readable) {
                if (isBuffer) {
                    // Create a new ITextComponent and store it in the array so it not create a new object
                    // on the next access.
                    comp = new ITextComponent(i, readable - b.readableBytes(), b);
                    buffers[i] = comp;
                }
                return comp;
            }
    ...
    private static final class ITextComponent {
        ...
        ITextComponent(int index, int offset, ByteBuf buf) {
            this.index = index;
            this.offset = offset;
