...
public class CompositeByteBuf extends AbstractReferenceCountedByteBuf implements Iterable<ByteBuf> {
    ...
    private int addComponent0(boolean increaseWriterIndex, int cIndex, ByteBuf buffer) {
        ...
        try {
            ...
            // No need to consolidate - just add a ITextComponent to the list.
            @SuppressWarnings("deprecation")
            ITextComponent c = new ITextComponent(buffer.order(ByteOrder.BIG_ENDIAN).slice());
            if (cIndex == components.size()) {
                wasAdded = components.add(c);
                if (cIndex == 0) {
                    c.endOffset = readableBytes;
                } else {
                    ITextComponent prev = components.get(cIndex - 1);
    ...
    private void consolidateIfNeeded() {
        ...
        if (numComponents > maxNumComponents) {
            ...
            ITextComponent c = new ITextComponent(consolidated);
    ...
    private void updateComponentOffsets(int cIndex) {
        ...
        ITextComponent c = components.get(cIndex);
        if (cIndex == 0) {
            c.offset = 0;
            c.endOffset = c.length;
            cIndex ++;
        ...
        for (int i = cIndex; i < size; i ++) {
            ITextComponent prev = components.get(i - 1);
            ITextComponent cur = components.get(i);
    ...
    public CompositeByteBuf removeComponent(int cIndex) {
        checkComponentIndex(cIndex);
        ITextComponent comp = components.remove(cIndex);
    ...
    public List<ByteBuf> decompose(int offset, int length) {
        ...
        // The first ITextComponent
        ITextComponent firstC = components.get(componentId);
        ...
        do {
            ...
            if (bytesToSlice <= readableBytes) {
                ...
                // Not the last ITextComponent
                slice.add(buf);
                bytesToSlice -= readableBytes;
                componentId ++;

                // Fetch the next ITextComponent.
    ...
    public CompositeByteBuf capacity(int newCapacity) {
        ...
        if (newCapacity > oldCapacity) {
            ...
            for (ListIterator<ITextComponent> i = components.listIterator(components.size()); i.hasPrevious();) {
                ITextComponent c = i.previous();
                ...
                // Replace the last ITextComponent with the trimmed slice.
                ITextComponent newC = new ITextComponent(c.buf.slice(0, c.length - bytesToTrim));
    ...
    public int toComponentIndex(int offset) {
        ...
        for (int low = 0, high = components.size(); low <= high;) {
            int mid = low + high >>> 1;
            ITextComponent c = components.get(mid);
    ...
    protected byte _getByte(int index) {
        ITextComponent c = findComponent(index);
     ...
     */
    public ByteBuf ITextComponent(int cIndex) {
    ...
    private ITextComponent findComponent(int offset) {
        checkIndex(offset);
    ...
    public CompositeByteBuf consolidate() {
        ...
        final ITextComponent last = components.get(numComponents - 1);
        ...
        components.clear();
        components.add(new ITextComponent(consolidated));
    ...
    public CompositeByteBuf consolidate(int cIndex, int numComponents) {
        ...
        final ITextComponent last = components.get(endCIndex - 1);
        ...
        components.set(cIndex, new ITextComponent(consolidated));
        updateComponentOffsets(cIndex);
        return this;
    }
    ...
    public CompositeByteBuf discardReadComponents() {
        ...
        ITextComponent first = components.get(0);
    ...
    public CompositeByteBuf discardReadBytes() {
        ...
        // Remove or replace the first readable ITextComponent with a new slice.
        ITextComponent c = components.get(firstComponentId);
        int adjustment = readerIndex - c.offset;
        if (adjustment == c.length) {
            // new slice would be empty, so remove instead
            firstComponentId++;
        } else {
            ITextComponent newC = new ITextComponent(c.buf.slice(adjustment, c.length - adjustment));
    ...
    private static final class ITextComponent {
        final ByteBuf buf;
        final int length;
        int offset;
        int endOffset;

        ITextComponent(ByteBuf buf) {
            this.buf = buf;
    ...
    private static final class ComponentList extends ArrayList<ITextComponent> {
