...
public final class ThreadLocalRandom extends Random {
    ...
    static {
        ...
        // The browser runtime has no /dev/random and cannot spawn the
        // SecureRandom seed generator thread, so we always seed from the
        // current time and skip the LinkedBlockingQueue/thread bookkeeping.
    ...
    public static long getInitialSeedUniquifier() {
        ...
        synchronized (ThreadLocalRandom.class) {
            ...
            // The browser runtime has no SecureRandom seed generator thread,
            // so we always derive the seed from the current time. Set the
            // interrupted flag to false so the existing post-loop bookkeeping
            // below stays in sync with the simpler seeding path.
            ...
            // interrupted is always false in the browser path; the seed
            // generator thread does not exist so nothing needs to be
            // interrupted or measured.
