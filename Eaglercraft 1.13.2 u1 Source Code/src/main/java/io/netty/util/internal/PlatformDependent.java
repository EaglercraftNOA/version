 ...
 * environment, such as Java version and direct memory support.
 * <p>
 * You can disable direct memory support if you specify
...
public final class PlatformDependent {
    ...
    /**
     * Return {@code true} if direct memory access can be used for accelerated
    ...
    /**
     * Return the reason (if any) why direct memory access was not available.
     */
    ...
    public static ByteBuffer directBuffer(long memoryAddress, int size) {
        ...
        throw new UnsupportedOperationException(
                "direct memory buffers are not available");
    }
    ...
    public static <T> Queue<T> newMpscQueue(final int maxCapacity) {
        return new ArrayDeque<T>(Math.max(1, maxCapacity));
    }
    ...
    public static <T> Queue<T> newFixedMpscQueue(int capacity) {
        return new ArrayDeque<T>(Math.max(1, capacity));
    }

    /**
     * Return the runtime loader token for the given {@link Class}.
     */
    public static Object getLoaderToken(final Class<?> clazz) {
        return PlatformDependent0.getLoaderToken(clazz);
    }

    /**
     * Return the context loader token for the current {@link Thread}.
     */
    public static Object getContextLoaderToken() {
        return PlatformDependent0.getContextLoaderToken();
    }

    /**
     * Return the system loader token.
     */
    public static Object getSystemLoaderToken() {
        return PlatformDependent0.getSystemLoaderToken();
    }
    ...
    private static Throwable unsafeUnavailabilityCause0() {
        if (isAndroid()) {
            logger.debug("direct memory: unavailable (Android)");
            return new UnsupportedOperationException("direct memory: unavailable (Android)");
        }
        ...
        try {
            ...
            logger.debug("direct memory: {}", hasUnsafe ? "available" : "unavailable");
    ...
    private static long maxDirectMemory0() {
        long maxDirectMemory = EagRuntime.maxMemory();
        logger.debug("maxDirectMemory: {} bytes (TeaVM)", maxDirectMemory);
