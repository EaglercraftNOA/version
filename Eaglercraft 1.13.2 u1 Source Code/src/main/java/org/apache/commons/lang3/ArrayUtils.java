...
public class ArrayUtils {
	...
	public static <T> T[] addAll(T[] arr1, T[] arr2) {
		T[] clone = Arrays.copyOf(arr1, arr1.length + arr2.length);
	...
	public static <T> T[] subarray(T[] array, int i, int j) {
		if (array == null) {
			return null;
		}
		if (i < 0) {
	...
	public static <T> T[] add(T[] array, int index, T element) {
		...
		T[] ret;
		if (array != null) {
			ret = Arrays.copyOf(array, length + 1);
			System.arraycopy(array, 0, ret, 0, index);
			System.arraycopy(array, index, ret, index + 1, length - index);
		} else {
			@SuppressWarnings("unchecked")
