 ...
 * {@code reflectionCompare} uses normal object comparison in this runtime to
...
public class CompareToBuilder implements Builder<Integer> {
    ...
    public static int reflectionCompare(
        ...
        if (lhs instanceof Comparable && lhs.getClass().isInstance(rhs)) {
            @SuppressWarnings({ "rawtypes", "unchecked" })
            int result = ((Comparable) lhs).compareTo(rhs);
            return result;
        }
        return lhs.equals(rhs) ? 0 : lhs.toString().compareTo(rhs.toString());
    }
    ...
    public static int reflectionCompare(final Object lhs, final Object rhs, final Collection<String> excludeFields) {
        return reflectionCompare(lhs, rhs,
                excludeFields == null ? new String[0] : excludeFields.toArray(new String[excludeFields.size()]));
    }
