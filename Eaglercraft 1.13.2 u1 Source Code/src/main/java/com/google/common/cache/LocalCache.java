...
import com.arlenh7.eaglercraft.v1_17.java.util.concurrent.ConcurrentLinkedQueue;
import java.util.concurrent.ConcurrentMap;
import java.util.concurrent.ExecutionException;
import java.util.concurrent.TimeUnit;
import java.util.concurrent.atomic.AtomicInteger;
import com.arlenh7.eaglercraft.v1_17.java.util.concurrent.atomic.AtomicReferenceArray;
import com.arlenh7.eaglercraft.v1_17.java.util.concurrent.locks.ReentrantLock;
...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
  ...
  private static <V> V getUninterruptibly(ListenableFuture<V> future) throws ExecutionException {
    boolean interrupted = false;
    try {
      while (true) {
        try {
          return future.get();
        } catch (net.lax1dude.eaglercraft.v1_8.futures.ExecutionException e) {
  ...
  static class LoadingValueReference<K, V> implements ValueReference<K, V> {
    ...
    public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
      try {
        ...
        SettableFuture<V> transformed = SettableFuture.create();
        newValue.addListener(
            () -> {
              try {
                V reloadedValue = getUninterruptibly(newValue);
                LoadingValueReference.this.set(reloadedValue);
                transformed.set(reloadedValue);
              } catch (Throwable t) {
                transformed.setException(
                    t instanceof ExecutionException && t.getCause() != null ? t.getCause() : t);
              }
            },
            Runnable::run);
        return transformed;
