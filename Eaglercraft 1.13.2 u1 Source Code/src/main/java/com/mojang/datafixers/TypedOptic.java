...
public final class TypedOptic<S, T, A, B> {
    protected final Set<OpticProof> proofBounds;
    ...
    public TypedOptic(final OpticProof proofBound, final Type<S> sType, final Type<T> tType, final Type<A> aType, final Type<B> bType, final Optic<?, S, T, A, B> optic) {
        this(ImmutableSet.of(proofBound), sType, tType, aType, bType, optic);
    }

    public TypedOptic(final Set<OpticProof> proofBounds, final Type<S> sType, final Type<T> tType, final Type<A> aType, final Type<B> bType, final Optic<?, S, T, A, B> optic) {
    ...
    public <P extends K2, Proof2 extends K1> App2<P, S, T> apply(final OpticProof token, final App<Proof2, P> proof, final App2<P, A, B> argument) {
        return upCast(token)
            .orElseThrow(() -> {
    ...
    public <A1, B1> TypedOptic<S, T, A1, B1> compose(final TypedOptic<A, B, A1, B1> other) {
        final ImmutableSet.Builder<OpticProof> builder = ImmutableSet.builder();
    ...
    public <Proof2 extends K1> Optional<Optic<? super Proof2, S, T, A, B>> upCast(final OpticProof proof) {
    ...
    public static boolean instanceOf(final Collection<OpticProof> bounds, final OpticProof proof) {
