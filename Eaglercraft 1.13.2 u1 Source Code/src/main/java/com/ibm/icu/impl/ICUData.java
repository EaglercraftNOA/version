...
public final class ICUData {
    ...
    private static InputStream getStream(final Object loader, final String resourceName, boolean required) {
        InputStream i = null;
        final String resolvedName = resourceName.startsWith("/") ? resourceName : "/" + resourceName;
        if (System.getSecurityManager() != null) {
            i = AccessController.doPrivileged(new PrivilegedAction<InputStream>() {
                    public InputStream run() {
                        return ICUData.class.getResourceAsStream(resolvedName);
                    }
                });
        } else {
            i = ICUData.class.getResourceAsStream(resolvedName);
        }
        if (i == null && required) {
            throw new MissingResourceException("could not locate data", String.valueOf(loader), resourceName);
        }
        return i;
    }
    
    public static InputStream getStream(Object loader, String resourceName){
        return getStream(loader,resourceName, false);   
    }

    public static InputStream getRequiredStream(Object loader, String resourceName){
