...
tasks.withType<JavaCompile>().configureEach {
	val task = this as org.gradle.api.tasks.compile.JavaCompile
	task.options.isFork = true
	classpath = classpath.filter { it.name.startsWith("lwjgl") } +
			classpath.filter { !it.name.startsWith("lwjgl") }
}
...
tasks.register<JavaExec>("eaglercraftDebugRuntime") {
	...
	classpath = runtimeClasspath.filter { it.name.startsWith("lwjgl") } +
			runtimeClasspath.filter { !it.name.startsWith("lwjgl") }
